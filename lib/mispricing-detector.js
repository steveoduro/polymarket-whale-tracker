...
class MispricingDetector {
  ...
  generatePositions(opportunity, capital, config = {}) {
    ...
    // Apply per-opportunity sizing overrides (bounded vs unbounded range types)
    // Single-threaded Node.js — save/restore is safe
    const savedKelly = this.kellyFraction;
    const savedMaxPos = this.maxPositionPct;
    if (config.kellyFraction) this.kellyFraction = config.kellyFraction;
    if (config.maxPositionPct) this.maxPositionPct = config.maxPositionPct;
    ...
    // Restore defaults
    this.kellyFraction = savedKelly;
    this.maxPositionPct = savedMaxPos;
