...
class MarketScanner {
  constructor(config = {}) {
    ...
    this.kalshiApi = this.kalshiEnabled ? new KalshiAPI({
      log: this.log,
      demo: config.kalshiDemo,
      apiKeyId: config.kalshiApiKey,
  ...
  parseEvent(event, cityInfo, date, dateStr) {
    try {
      ...
      const ranges = event.markets.map(market => {
        ...
        // Use bestAsk as price (what we'd pay to buy)
        // Fall back to outcomePrices only if bestAsk is 0
        let price = bestAsk;
  ...
  parsePrecipitationEvent(event, slugInfo) {
    try {
      ...
      const ranges = event.markets.map(market => {
        ...
        // Use bestAsk as primary price (we're buyers, so we pay the ask)
        // Fall back to outcomePrices only if bestAsk is 0
        let price = bestAsk;
        if (price === 0 && market.outcomePrices) {
          try {
  ...
  async discoverWeatherMarkets() {
    try {
      ...
      const resp = await fetch(`${GAMMA_API}/events?limit=100&closed=false`);
      if (!resp.ok) return null;

      const events = await resp.json();
      if (!events || events.length === 0) return null;

      // Filter for actual weather markets by slug pattern
      const weatherEvents = events.filter(e => {
        const slug = (e.slug || '').toLowerCase();
        return slug.includes('temperature') ||
               slug.includes('precipitation') ||
               slug.startsWith('highest-temperature-') ||
               slug.startsWith('precipitation-in-');
      });

      if (weatherEvents.length === 0) {
        return { total: 0, unknown: 0 };
      }
      ...
      const unknownSlugs = weatherEvents
      ...
      return { total: weatherEvents.length, unknown: unknownSlugs.length };
    } catch (err) {
  ...
  /**
   * Get Kalshi API instance for trading operations
   */
  getKalshiApi() {
    return this.kalshiApi;
  }

  /**
   * Check if Kalshi trading is enabled (has credentials)
   */
  isKalshiTradingEnabled() {
    return this.kalshiApi?.isTradingEnabled() || false;
  }
}
