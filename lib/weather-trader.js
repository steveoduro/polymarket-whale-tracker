...
class WeatherTrader {
    ...
    try {
      ...
      if (opportunity) {
        ...
        const oppRecord = {
          ...
          fee_adjusted_ev: opportunity.expectedValue?.evPct ?? null,
          status: 'traded',
          platform: platform,
  ...
  async resolveTemperatureTrade(trade, weatherApi) {
    ...
    if (platform === 'kalshi') {
      actual = await weatherApi.getKalshiResolutionHigh(trade.city, trade.target_date);
    } else {
      actual = await weatherApi.getHistoricalHigh(trade.city, trade.target_date);
    }

    if (!actual) {
      this.log('warn', 'Could not get actual temp', { city: trade.city, date: trade.target_date, platform });
      return null;
    }

    this.log('info', 'Resolution source', {
      city: trade.city, platform, source: actual.source,
      stationId: actual.stationId || 'N/A',
    });
