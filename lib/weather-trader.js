...
class WeatherTrader {
  ...
  async savePaperTrade(trade, opportunity) {
    ...
    try {
      ...
      // Save trade (including forecast and hedge info)
      const tradeRecord = {
  ...
  async resolveTemperatureTrade(trade, weatherApi) {
    ...
    // Record forecast accuracy (pass weatherApi for the method)
  ...
  async resolvePrecipitationTrade(trade, weatherApi) {
    ...
    // Get the market ranges to find winning range
    ...
    if (opp && opp.ranges) {
      ...
      for (const range of ranges) {
        if (this.precipMatchesRange(actualInches, range.name)) {
