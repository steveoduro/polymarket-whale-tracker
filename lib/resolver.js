  ...
  async _updatePwsStationBias() {
    ...
    try {
      ...
      for (const row of rows) {
        ...
        // Compute distance to METAR station if we have lat/lon
        let distKm = null;
        const cityConfig = config.cities[row.city];
        if (row.lat != null && row.lon != null && cityConfig) {
          // Use city center as METAR proxy — close enough for weighting
          const metarLat = cityConfig.lat;
          const metarLon = cityConfig.lon;
