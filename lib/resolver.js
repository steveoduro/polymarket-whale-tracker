...
const { db, dbAdmin, execSQL } = require('./db');
...
class Resolver {
  constructor(forecastEngine, alerts) {
    ...
    // Retention cleanup tracking (runs once per 24h)
    this._lastRetentionCleanup = 0;
  ...
  async resolve() {
    ...
    // 30-day retention cleanup for opportunities table (once per 24h)
    try {
      const RETENTION_CLEANUP_INTERVAL = 24 * 60 * 60 * 1000;
      if (Date.now() - this._lastRetentionCleanup >= RETENTION_CLEANUP_INTERVAL) {
        const cutoff = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString();
        const client = dbAdmin || db;
        const { count, error: delErr } = await client
          .from('opportunities')
          .delete()
          .lt('created_at', cutoff);
        if (!delErr) {
          this._log('info', 'Retention cleanup: deleted opportunities older than 30 days', { count });
        } else {
          this._log('warn', 'Retention cleanup failed', { error: delErr.message });
        }
        this._lastRetentionCleanup = Date.now();
      }
    } catch (err) {
      this._log('warn', 'Retention cleanup failed', { error: err.message });
    }
