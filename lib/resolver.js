...
class Resolver {
  ...
  async _resolveTrades() {
    ...
    for (const trade of trades) {
      try {
        // Check if another trade for this city/date already resolved with an actual_temp
        ...
        if (existingResolved && existingResolved.length > 0) {
          ...
          actual = await this._getActualHigh(trade.city, trade.target_date);
        }
  ...
  async _backfillOpportunities() {
    ...
    for (const [key, groupOpps] of groups) {
      ...
      // Check if a trade for this city/date already resolved with an actual_temp
      let actual;
      const { data: existingResolved } = await db
        .from('trades')
        .select('actual_temp, range_unit')
      ...
      if (existingResolved && existingResolved.length > 0) {
        ...
        actual = await this._getActualHigh(city, dateStr);
      }
  ...
  async _recordAccuracy() {
    ...
    // Query distinct city/date combos with resolved actuals and valid model data
