...
// Weather Underground base — Polymarket's actual resolution source
const WU_HISTORY_BASE = 'https://www.wunderground.com/history/daily';

// WU URL patterns per city (from Polymarket contract rules)
const WU_CITY_PATHS = {
...
class Resolver {
  ...
  async _resolveTrades() {
    ...
    for (const trade of trades) {
      try {
        ...
        // Log WU audit URL for Polymarket trades (manual spot-check against canonical source)
        const wuUrl = this._getWUUrl(trade.city, trade.target_date, trade.platform);
  ...
  /**
   * Build Weather Underground History URL for a city/date (Polymarket's resolution source).
   * Returns null for non-Polymarket trades or unknown cities.
   */
  _getWUUrl(city, dateStr, platform) {
    if (platform !== 'polymarket') return null;
    ...
    // Format: /history/daily/{path}/{STATION}/date/YYYY-M-D
  ...
  async _recordAccuracy() {
    ...
    const { data: opps, error: oppErr } = await db
      .from('opportunities')
      .select('city, target_date, actual_temp, forecast_sources, forecast_confidence, range_unit, ensemble_std_dev')
      .not('actual_temp', 'is', null)
  ...
  async _getActualHigh(city, dateStr, platform) {
    ...
    if (platform === 'kalshi') {
      // Kalshi: use nwsStation first (NWS API), then polymarketStation via METAR as fallback
      ...
      // Polymarket (or unknown): use polymarketStation via METAR first, then nwsStation as fallback
      if (cityConfig.polymarketStation) {
        result = await this._getMETARHigh(cityConfig.polymarketStation, dateStr, cityConfig.tz);
        if (result) stationUsed = cityConfig.polymarketStation;
      }
      if (!result && cityConfig.nwsStation) {
        // Fallback to NWS if METAR failed — only if nwsStation differs from polymarketStation
        result = await this._getNWSObservationHigh(cityConfig.nwsStation, dateStr, cityConfig.tz);
        if (result) stationUsed = cityConfig.nwsStation;
      }
    }
