  ...
  async _resolveTrades() {
    ...
    for (const trade of trades) {
      try {
        ...
        const feeRate = config.platforms[trade.platform]?.feeRate || 0;

        // Calculate P&L (fees are on profit only — Polymarket/Kalshi)
        let pnl, fees;
        if (won) {
          // Win: receive $1 per share, fee on profit
          const revenue = trade.shares * 1.0;
          const profit = revenue - trade.cost;
          fees = Math.max(0, profit * feeRate);
          pnl = profit - fees;
        } else {
          // Loss: shares expire worthless, no fees (no profit)
          fees = 0;
          pnl = -trade.cost;
        }
