...
class Resolver {
  ...
  async resolve() {
    ...
    try {
      ...
      if (stats.opportunitiesBackfilled > 0) {
  ...
  /**
   * Backfill won + actual_temp on exited/resolved trades where won IS NULL.
   * These are trades the monitor closed (guaranteed_loss/win, edge_gone, take_profit)
  ...
  async _backfillExitedTrades() {
    ...
    for (const trade of pastTrades) {
      try {
        ...
        // Check if another trade already resolved with an actual_temp
        const { data: existingResolved } = await db
          .from('trades')
          .select('actual_temp, range_unit')
          .eq('city', trade.city)
        ...
        const { error: updateError } = await db
          .from('trades')
          .update({ won, actual_temp: actualTemp })
          .eq('id', trade.id);
    ...
    if (backfilled > 0) {
      this._log('info', `Backfilled won/actual_temp on ${backfilled} exited trades`);
    }
  ...
  async _refreshCalibrationTable() {
    try {
      await execSQL(`
        ...
        INSERT INTO market_calibration (range_type, lead_time_bucket, price_bucket, n, empirical_win_rate, avg_model_prob, calibration_gap, avg_ask, true_edge)
        SELECT
          range_type,
          CASE
          ...
          CASE
            ...
            WHEN ask < 0.35 THEN '25-35c'
            ELSE '35c+'
          END as price_bucket,
          COUNT(*) as n,
          AVG(CASE WHEN would_have_won THEN 1.0 ELSE 0 END) as empirical_win_rate,
          AVG(our_probability) as avg_model_prob,
          AVG(our_probability) - AVG(CASE WHEN would_have_won THEN 1.0 ELSE 0 END) as calibration_gap,
          AVG(ask) as avg_ask,
          AVG(CASE WHEN would_have_won THEN 1.0 ELSE 0 END) - AVG(ask) as true_edge
        FROM opportunities
        WHERE side = 'YES'
      ...
      await execSQL(`
        ...
        SELECT
          ...
          CASE
            ...
            WHEN our_probability < 0.50 THEN '35-50%'
            ELSE '50%+'
          END as model_prob_bucket,
          COUNT(*) as n,
          AVG(our_probability) as avg_model_prob,
          AVG(CASE WHEN would_have_won THEN 1.0 ELSE 0 END) as actual_win_rate,
          CASE
            WHEN AVG(our_probability) > 0
            THEN AVG(CASE WHEN would_have_won THEN 1.0 ELSE 0 END) / AVG(our_probability)
            ELSE 0
