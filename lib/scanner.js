...
class Scanner {
  ...
  _evaluateYes(city, dateStr, range, forecast) {
    ...
    // Apply model calibration correction (capped to prevent extreme tail corrections)
    ...
    const maxRatio = config.calibration?.MAX_CORRECTION_RATIO || 3.0;
    const correctionRatio = Number.isFinite(rawRatio) ? Math.min(rawRatio, maxRatio) : 1.0;
  ...
  async scanGuaranteedWins() {
    ...
    for (const [cityKey, cityConfig] of Object.entries(config.cities)) {
      ...
      for (const range of ranges) {
        ...
        if (!dualConfirmed || isKalshi) {
          ...
          // For Polymarket: use min(METAR, WU) when WU available — WU is resolution source.
          // Prevents METAR-only entries where WU disagrees (e.g. Seoul: METAR=13, WU=12, threshold=12.5).
          const gapSource = (isPolymarket && confirmAvailable && confirmHigh != null)
            ? Math.min(primaryHigh, confirmHigh)
            : primaryHigh;
          const gap = gapSource - threshold;
          if (gap < minGap) {
            missed.push({
              city: cityKey, target_date: localToday, platform: range.platform,
              range_name: range.rangeName, side: signal.side, ask,
    ...
    if (newMissed.length > 0) this._persistMissedGW(newMissed);
  ...
  async evaluateGWFastPath(candidates) {
    ...
    const { data: openTrades } = await query(
      `SELECT city, target_date, range_name, range_min, side, platform, entry_reason
       FROM trades WHERE city = ANY($1) AND status = 'open'`,
      [cities]
    );
    const positionKeys = new Set();
    const pwsPositionKeys = new Set(); // PWS-specific dedup (PWS+METAR are independent)
    ...
    for (const t of (openTrades || [])) {
      ...
      if (t.entry_reason === 'guaranteed_win_pws') {
        pwsPositionKeys.add(`${t.city}:${t.target_date}:${t.range_name}:${t.side}:${t.platform}`);
      }
      if (t.side === 'YES') {
    ...
    // 2b. PWS cross-platform dedup: same city/range/side on multiple platforms → keep lowest ask
    // Use range_min:range_max as key (not range_name — differs across platforms: "78-79°F" vs "78° to 79°")
    const pwsCrossPlat = new Map(); // city:date:min:max:side → best candidate
    for (const c of candidates) {
      if (c.entry_reason_override !== 'guaranteed_win_pws') continue;
      const xKey = `${c.city}:${c.target_date}:${c.range_min}:${c.range_max}:${c.side}`;
      const existing = pwsCrossPlat.get(xKey);
      if (!existing || c.ask < existing.ask) {
        pwsCrossPlat.set(xKey, c);
      }
    }
    // Also check DB: if any platform already has a PWS trade for this city/range/side, skip all platforms
    const pwsCrossDbKeys = new Set();
    for (const t of (openTrades || [])) {
      if (t.entry_reason === 'guaranteed_win_pws') {
        pwsCrossDbKeys.add(`${t.city}:${t.target_date}:${t.range_min}:${t.range_max}:${t.side}`);
      }
    }

    // 3. Apply filter chain per candidate (same logic as scanGuaranteedWins)
    for (const c of candidates) {
      const isPws = c.entry_reason_override === 'guaranteed_win_pws';
      const gwConfig = isPws ? config.pws_gw : config.guaranteed_entry;

      // Skip ranges already held — no alert, no missed entry
      // PWS: cross-platform dedup — only enter on the platform with the lowest ask
      if (isPws) {
        const xKey = `${c.city}:${c.target_date}:${c.range_min}:${c.range_max}:${c.side}`;
        // Skip if any platform already has a PWS trade for this city/range/side
        if (pwsCrossDbKeys.has(xKey)) continue;
        // Skip if this isn't the best-ask platform in this batch
        const best = pwsCrossPlat.get(xKey);
      ...
      // Gap check — PWS uses its own MIN_GAP_F/C; METAR uses existing dual-station logic
      if (isPws) {
        const minGap = c.range_unit === 'C' ? (gwConfig.MIN_GAP_C || 0.5) : (gwConfig.MIN_GAP_F || 1.0);
        if (c.gap < minGap) {
          missed.push({
            ...
            reason: 'below_pws_gap', gap: Math.round(c.gap * 10) / 10, minGap, isPws: true,
          });
          continue;
        }
      } else {
      ...
      // ASK filters — PWS uses its own MIN_ASK/MAX_ASK
      const maxAsk = isPws ? gwConfig.MAX_ASK : gwConfig.MAX_ASK;
      const minAsk = isPws ? gwConfig.MIN_ASK : gwConfig.MIN_ASK;
      if (c.ask > maxAsk) {
        if (c.ask < 1.0) {
          missed.push({
            city: c.city, target_date: c.target_date, platform: c.platform,
            range_name: c.range_name, side: c.side, ask: c.ask,
            observation_high: c.effHigh, unit: c.range_unit,
            reason: 'above_max_ask', maxAsk, isPws,
          });
        }
        continue;
      }
      if (c.ask < minAsk) {
        missed.push({
          ...
          range_name: c.range_name, side: c.side, ask: c.ask,
          observation_high: c.effHigh, unit: c.range_unit,
          reason: 'below_min_ask', minAsk, isPws,
        });
        continue;
      }

      // Market sanity: bid must be meaningful
      const gwMinBid = isPws ? (gwConfig.MIN_BID || 0.10) : (gwConfig.GW_MIN_BID || 0.10);
      if (c.bid != null && c.bid < gwMinBid) {
        missed.push({
          city: c.city, target_date: c.target_date, platform: c.platform,
          range_name: c.range_name, side: c.side, ask: c.ask,
          observation_high: c.effHigh, unit: c.range_unit,
          reason: 'below_min_bid', bid: Math.round(c.bid * 100), minBid: Math.round(gwMinBid * 100), isPws,
        });
        continue;
      }

      // Fee/margin check (only for METAR — PWS doesn't require MIN_MARGIN_CENTS)
      const fee = this.adapter.getEntryFee(c.platform, c.ask);
      const margin = 1.00 - c.ask - fee;
      if (!isPws && margin * 100 < gwConfig.MIN_MARGIN_CENTS) continue;

      // Dedup — for PWS, only block if same strategy already has position
      if (!isPws) {
      ...
      if (isPws) {
        // PWS entry — no dual-confirmation needed
        entries.push({
          city: c.city,
          ...
          volume: c.volume,
          entry_reason: 'guaranteed_win_pws',
          observation_high: c.pws_corrected_median,
          wu_high: null,
          margin,
          gap: c.gap,
          dual_confirmed: false,
          wu_triggered: false,
          pws_corrected_median: c.pws_corrected_median,
          pws_stations_online: c.pws_stations_online,
          pws_avg_error: c.pws_avg_error,
          _freshBookAsk: c._freshBookAsk,
        });
      } else {
        // METAR entry — existing dual-confirmation logic
  ...
  /**
   * Persist missed GW entries to system_events for analysis.
   * Fire-and-forget — don't block the GW pipeline on DB writes.
   */
  _persistMissedGW(missed) {
    for (const m of missed) {
      query(
        `INSERT INTO system_events (event_type, event_name, details)
         VALUES ('gw_missed', $1, $2)`,
        [
          `${m.city}|${m.target_date}|${m.platform}|${m.range_name}|${m.side}`,
          JSON.stringify({
            city: m.city,
            target_date: m.target_date,
            platform: m.platform,
            range_name: m.range_name,
            side: m.side,
            ask: m.ask,
            reason: m.reason,
            observation_high: m.observation_high,
            unit: m.unit,
            isPws: m.isPws || false,
            gap: m.gap,
            minGap: m.minGap,
            ...
            bid: m.bid,
            minBid: m.minBid,
            dual_confirmed: m.dual_confirmed,
          }),
        ]
      ).catch(err => this._log('error', 'Failed to persist missed GW entry', { error: err.message }));
    }
  }
  ...
  _checkGuaranteedWin(range, primaryHigh, confirmHigh, requireDual, confirmAvailable) {
    ...
    // Bounded range ("X-Y") or unbounded lower ("X or below"):
  ...
  _findYesCandidates(ranges, forecast, city) {
    ...
    for (const range of ranges) {
      ...
      const rawRatio = (modelCal && modelCal.n >= 30) ? Number(modelCal.correction_ratio) : 1.0;
      const maxR = config.calibration?.MAX_CORRECTION_RATIO || 3.0;
      const ratio = Number.isFinite(rawRatio) ? Math.min(rawRatio, maxR) : 1.0;
