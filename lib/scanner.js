...
class Scanner {
          ...
          for (const range of candidates) {
            ...
            if (yesOpp.action === 'entered' && !bestApproved) {
              ...
              const calForKelly = (forecast.hoursToResolution != null)
                ? this._getCalibration(range.rangeType, forecast.hoursToResolution, yesOpp.ask) : null;
              if (calForKelly && calForKelly.n >= 50 && Number(calForKelly.empirical_win_rate) > yesOpp.ask
  ...
  _applyFilters(side, edgePct, spread, ask, bid, range, hoursToResolution) {
    ...
    const cal = (side === 'YES' && hoursToResolution != null)
      ? this._getCalibration(range.rangeType, hoursToResolution, ask) : null;
    const calConfirmsEdge = cal && cal.n >= 50 && Number(cal.empirical_win_rate) > ask;
    ...
    if (cal && cal.n >= 50 && Number(cal.empirical_win_rate) < ask) {
      reasons.push(
