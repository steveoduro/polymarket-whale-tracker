...
class Scanner {
  ...
  _applyFilters(side, edgePct, spread, ask, bid, range, hoursToResolution, forecast, city) {
    ...
    let calConfirmsEdge = cal && cal.n >= config.calibration.CAL_CONFIRMS_MIN_N && Number(cal.empirical_win_rate) > ask && edgePct >= 0;
  ...
  async _loadModelCalibration() {
    try {
      ...
      if (active > 0) {
        this._log('info', `Model calibration: ${active} buckets active (n≥30), ${belowThreshold} below threshold (ratio=1.0 fallback)`);
      } else if (map.size > 0) {
        this._log('warn', `Model calibration: 0 buckets active (all ${map.size} have n<30) — all ratios defaulting to 1.0`);
      }
      return map;
    } catch (err) {
