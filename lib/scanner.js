    ...
    for (const [cityKey, cityConfig] of Object.entries(config.cities)) {
      ...
      for (const dateStr of dates) {
        try {
          ...
          if (closestRange) {
            ...
            if (yesOpp) {
              ...
              if (yesOpp.action === 'entered' && obsGate.ENABLED && isBounded
                  ...
                  && localHour != null && localHour < (config.observer.COOLING_HOUR || 20)) {
                ...
                if (high != null && high > forecast.temp && high >= closestRange.rangeMax - buffer) {
                  yesOpp.action = 'filtered';
                  yesOpp.filter_reason = `obs_entry_gate (high ${high}° > forecast ${forecast.temp}° and ${(closestRange.rangeMax - high).toFixed(1)}° from ceiling ${closestRange.rangeMax}°, ${localHour}:00 local < cooling ${config.observer.COOLING_HOUR || 20}:00)`;
          ...
          // ── NO: evaluate all ranges (unchanged) ──
