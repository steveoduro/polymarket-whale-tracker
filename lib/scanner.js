...
class Scanner {
  ...
  _evaluateYes(city, dateStr, range, forecast) {
    ...
    // Kelly fraction: b = netProfit/ask (true net odds for prediction markets)
    const netProfit = payout - ask;
    const kellyFull = probability > 0 && netProfit > 0
      ? ((netProfit / ask) * probability - (1 - probability)) / (netProfit / ask)
      : 0;
  ...
  _evaluateNo(city, dateStr, range, forecast) {
    ...
    // Kelly fraction: b = netProfit/noAsk (true net odds for prediction markets)
    const noNetProfit = payout - noAsk;
    const kellyFull = noProbability > 0 && noNetProfit > 0
      ? ((noNetProfit / noAsk) * noProbability - (1 - noProbability)) / (noNetProfit / noAsk)
      : 0;
    const kelly = Math.max(0, kellyFull * config.sizing.KELLY_FRACTION);
