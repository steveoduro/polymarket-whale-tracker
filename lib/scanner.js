...
class Scanner {
  ...
  _forecastToNearEdge(temp, rangeMin, rangeMax) {
    ...
    if (rangeMin != null && rangeMax != null) {
      ...
      if (dMin < 0) return dMin;     // below range_min
      if (dMax < 0) return -dMax;    // above range_max (flip sign: positive = distance TO boundary)
      return -Math.min(dMin, dMax);  // inside range, negative = how far past the nearer edge
    }
    if (rangeMin != null && rangeMax == null) {
      // Unbounded upper ("X or higher"): distance to rangeMin
      return temp - rangeMin;
    }
    if (rangeMin == null && rangeMax != null) {
      // Unbounded lower ("X or below"): distance to rangeMax
      return rangeMax - temp;
    }
    return null;
  }
  ...
  _forecastToFarEdge(temp, rangeMin, rangeMax) {
    ...
    if (dMin < 0) return rangeMax - temp;   // below range: far edge is rangeMax
    if (dMax < 0) return temp - rangeMin;   // above range: far edge is rangeMin (positive = far)
    return Math.max(dMin, dMax);            // inside: farther boundary distance
  }
  ...
  async evaluateGWFastPath(candidates) {
    ...
    const { data: openTrades } = await query(
      `SELECT city, target_date, range_name, side, platform
    ...
    for (const t of (openTrades || [])) {
      ...
      if (t.side === 'YES') openYesKeys.add(`${t.city}:${t.target_date}:${t.platform}`);
    ...
    for (const c of candidates) {
      ...
      entries.push({
        ...
        ask: c.ask,
        volume: c.volume,
        entry_reason: 'guaranteed_win_metar_only',
        observation_high: c.effHigh,
        margin,
        dual_confirmed: false,
        wu_triggered: c.wuTriggered || wuTriggeredSet.has(wuKey),
