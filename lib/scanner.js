...
class Scanner {
  ...
  _applyFilters(side, edgePct, spread, ask, bid, range) {
    ...
    if (ask < 0.03) {
      reasons.push(`ask ${(ask * 100).toFixed(1)}¢ too low (< 3¢, likely no liquidity)`);
    }
    if (ask >= 0.97) {
    ...
    if (range.volume === 0 && (range.liquidity === 0 || range.liquidity === undefined)) {
      reasons.push('zero volume/liquidity');
    }
