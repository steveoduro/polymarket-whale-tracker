...
class Scanner {
  ...
  async scanGuaranteedWins() {
      ...
      for (const range of ranges) {
        ...
        if (!dualConfirmed || isKalshi) {
          ...
          const minGap = cityConfig.unit === 'C'
            ? (isKalshi ? config.guaranteed_entry.METAR_ONLY_MIN_GAP_C_KALSHI : config.guaranteed_entry.METAR_ONLY_MIN_GAP_C)
            : (isKalshi ? config.guaranteed_entry.METAR_ONLY_MIN_GAP_F_KALSHI : config.guaranteed_entry.METAR_ONLY_MIN_GAP_F);
    ...
    // Debounce missed alerts — only return new misses not yet alerted today
    const today = new Date().toISOString().split('T')[0];
    if (this._missedAlertedDate !== today) {
      this._missedAlerted = new Set();
      this._missedAlertedDate = today;
    }
    const newMissed = missed.filter(m => {
      const key = `${m.city}_${m.target_date}_${m.platform}_${m.range_name}_${m.side}`;
