...
class Scanner {
        ...
        try {
          ...
          for (const range of candidates) {
            ...
            if (yesOpp.action === 'entered' && !bestApproved) {
              ...
              if (calForKelly && calForKelly.n >= config.calibration.CAL_CONFIRMS_MIN_N && Number(calForKelly.empirical_win_rate) > yesOpp.ask
  ...
  _evaluateYes(city, dateStr, range, forecast) {
    ...
    const modelCal = this._getModelCalibration(range.rangeType, rawProbability);
    const correctionRatio = (modelCal && modelCal.n >= 30) ? Number(modelCal.correction_ratio) : 1.0;
    ...
    const { action, filterReason } = this._applyFilters('YES', edgePct, spread, ask, bid, range, forecast.hoursToResolution, forecast, city);
  ...
  _evaluateNo(city, dateStr, range, forecast) {
    ...
    // Determine action + filter reason
    const { action, filterReason } = this._applyFilters('NO', edgePct, spread, noAsk, 1 - ask, range, forecast.hoursToResolution, forecast, city);
   ...
   */
  _applyFilters(side, edgePct, spread, ask, bid, range, hoursToResolution, forecast, city) {
    const reasons = [];
    ...
    let calConfirmsEdge = cal && cal.n >= config.calibration.CAL_CONFIRMS_MIN_N && Number(cal.empirical_win_rate) > ask;
  ...
  async _logOpportunity(opp) {
    try {
      const { data, error } = await db.from('opportunities').insert({
        ...
        corrected_probability: opp.corrected_probability || null,
        correction_ratio: opp.correction_ratio || null,
      }).select('id').single();
  ...
  async scanGuaranteedWins() {
    ...
    for (const [cityKey, cityConfig] of Object.entries(config.cities)) {
      ...
      for (const range of ranges) {
        ...
        // Polymarket resolves via WU — use wu_high as primary (same pattern as monitor guaranteed_loss)
        // Kalshi resolves via NWS/METAR — running_high is appropriate
        const isPolymarket = range.platform === 'polymarket';
        let primaryHigh, confirmHigh, confirmAvailable;
        if (isPolymarket) {
          if (wuHigh == null) continue;  // WU unavailable — skip rather than use METAR-only
          primaryHigh = wuHigh;
          confirmHigh = runHigh;         // running_high as confirmation (always >= wuHigh)
          confirmAvailable = runHigh != null;
        } else {
          primaryHigh = runHigh;
          confirmHigh = wuHigh;          // WU as confirmation for Kalshi
        ...
        entries.push({
          ...
          wu_high: wuHigh,
          dual_confirmed: confirmAvailable && confirmHigh != null && this._bothCrossThreshold(signal, primaryHigh, confirmHigh),
          margin,
        });
      }
    }
  ...
  _findYesCandidates(ranges, forecast, city) {
    ...
    for (const range of ranges) {
      ...
      const modelCal = this._getModelCalibration(range.rangeType, rawProb);
      const ratio = (modelCal && modelCal.n >= 30) ? Number(modelCal.correction_ratio) : 1.0;
  ...
  async _loadCityCalibration() {
    try {
      const { data, error } = await db
        ...
        .gt('ask', 0)
        .not('hours_to_resolution', 'is', null)
        .or('model_valid.is.null,model_valid.eq.true');
  ...
  async _loadModelCalibration() {
    try {
      const { data, error } = await db.from('model_calibration').select('*');
      if (error) throw error;

      const map = new Map();
      for (const row of (data || [])) {
        const key = `${row.range_type}|${row.model_prob_bucket}`;
        map.set(key, row);
      }
      if (map.size > 0) {
        this._log('info', `Loaded ${map.size} model calibration buckets`);
      }
      return map;
    } catch (err) {
  ...
  _getModelCalibration(rangeType, rawProbability) {
    ...
    const key = `${rangeType}|${bucket}`;
    return this._modelCalibration.get(key) || null;
  }
}
