...
class Scanner {
  ...
  async evaluateGWFastPath(candidates) {
    ...
    for (const c of candidates) {
      ...
      // Skip ranges already held — no alert, no missed entry
      // For PWS: only skip if there's already a PWS trade on this range (METAR trades don't block PWS)
      if (!isPws) {
      ...
      if (isPws) {
        ...
        if (c.gap < minGap) {
          missed.push({
            ...
            reason: 'below_pws_gap', gap: Math.round(c.gap * 10) / 10, minGap,
          });
          continue;
        }
      } else {
