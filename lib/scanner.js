...
class Scanner {
  ...
  async evaluateGWFastPath(candidates) {
    ...
    const pwsCrossPlat = new Map(); // city:date:range:side → best candidate
    for (const c of candidates) {
      if (c.entry_reason_override !== 'guaranteed_win_pws') continue;
      const xKey = `${c.city}:${c.target_date}:${c.range_name}:${c.side}`;
    ...
    for (const t of (openTrades || [])) {
      if (t.entry_reason === 'guaranteed_win_pws') {
        pwsCrossDbKeys.add(`${t.city}:${t.target_date}:${t.range_name}:${t.side}`);
      }
    }
