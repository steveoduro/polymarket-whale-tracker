...
class Scanner {
  ...
  _applyFilters(side, edgePct, spread, ask, bid, range, hoursToResolution) {
    ...
    if (hoursToResolution != null && hoursToResolution < config.entry.MIN_HOURS_TO_RESOLUTION) {
      reasons.push(`too_close_to_resolution (${hoursToResolution.toFixed(1)}h < ${config.entry.MIN_HOURS_TO_RESOLUTION}h)`);
    }
    if (hoursToResolution === 0 || hoursToResolution === null) {
      reasons.push('hours_to_resolution_unknown_or_zero');
    }
