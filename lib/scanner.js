...
class Scanner {
  ...
  _evaluateYes(city, dateStr, range, forecast) {
    ...
    const { action, filterReason } = this._applyFilters('YES', edgePct, spread, ask, bid, range, forecast.hoursToResolution, forecast);
  ...
  _evaluateNo(city, dateStr, range, forecast) {
    ...
    // Determine action + filter reason
    const { action, filterReason } = this._applyFilters('NO', edgePct, spread, noAsk, 1 - ask, range, forecast.hoursToResolution, forecast);
   ...
   */
  _applyFilters(side, edgePct, spread, ask, bid, range, hoursToResolution, forecast) {
    const reasons = [];
    ...
    const calConfirmsEdge = cal && cal.n >= config.calibration.CAL_CONFIRMS_MIN_N && Number(cal.empirical_win_rate) > ask;
