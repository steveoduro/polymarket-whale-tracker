...
class Monitor {
  ...
  async _evaluateSingle(trade) {
    ...
    if (shouldExit && trade.side === 'YES' && forecast?.hoursToResolution != null) {
      const cal = this._getCalibration(trade.range_type, forecast.hoursToResolution, trade.entry_ask);
      if (cal && cal.n >= 50 && Number(cal.empirical_win_rate) > marketBid) {
        shouldExit = false;
        calOverride = {
          emp_win_rate: Number(cal.empirical_win_rate),
          bucket: `${trade.range_type}|${cal.lead_time_bucket}|${cal.price_bucket}`,
          n: cal.n,
        };
      }
    }
    ...
    if (obs) {
      ...
      const decision = this._checkAlreadyDecided(trade, obs, localHour, isToday);
      logEntry.observation_signal = decision;

      if (decision === 'guaranteed_loss') {
        const coolingHour = peakHours.get(trade.city.toLowerCase());
        const isDayOverLoss = isToday && localHour != null && localHour >= coolingHour;
        // Show the source that triggered: wu_high for "exceeded", running_high for "day-over"
        ...
        const triggerHigh = isDayOverLoss ? runH : (wuH ?? runH);
        const triggerSource = isDayOverLoss ? 'max' : 'WU';
        logEntry.action = 'recommend_exit';
        logEntry.exit_type = 'guaranteed_loss';
        logEntry.wu_high = wuH;
        logEntry.reason = isDayOverLoss
          ? `Day over (${localHour}:00 local): high ${triggerHigh}° never reached ${trade.range_name}`
          : `WU high ${triggerHigh}° already excludes ${trade.range_name}`;
   ...
   * Platform-aware for guaranteed_loss:
   *   - "exceeded" (high > boundary): use wu_high only
   *     Polymarket resolves via WU; Kalshi "both agree" since running_high ≥ wu_high
   *   - "didn't reach" (day-over, high < boundary): use running_high (Math.max)
   *     If the max of both sources hasn't reached, neither has the resolution source
   * guaranteed_win always uses running_high (Math.max — overshooting is safe)
   * Returns 'guaranteed_win' | 'guaranteed_loss' | 'undecided'
   */
  _checkAlreadyDecided(trade, obs, localHour, isToday) {
    ...
    if (trade.side === 'YES') {
      ...
      // Use running_high (Math.max) — if the max hasn't reached, neither source has
      if (dayOver) {
        // YES unbounded upper ("X or higher"): runHigh < threshold, day over = guaranteed loss
        if (trade.range_max == null && trade.range_min != null && runHigh < trade.range_min) {
          return 'guaranteed_loss';
        }
        // YES bounded ("X-Y"): runHigh < range_min, day over = guaranteed loss
        if (trade.range_max != null && trade.range_min != null && runHigh < trade.range_min) {
          return 'guaranteed_loss';
        }
      }
    ...
    if (trade.side === 'NO') {
      ...
      // ── Day-over cases ──
      if (dayOver) {
        // NO bounded: runHigh < range_min, day over = guaranteed win
        if (trade.range_max != null && trade.range_min != null && runHigh < trade.range_min) {
          return 'guaranteed_win';
        }
        // NO unbounded upper ("X or higher"): runHigh < threshold, day over = guaranteed win
        if (trade.range_max == null && trade.range_min != null && runHigh < trade.range_min) {
          return 'guaranteed_win';
        }
      }
  ...
  async _resolveGuaranteed(trade, signal, currentProbability, forecast, obs) {
    ...
    const actualTemp = trade.range_unit === 'C'
      ? (runningHighC ?? null)
      : (runningHighF ?? null);
    ...
    const exitData = {
      ...
      wu_high: trade.range_unit === 'C' ? wuHighC : wuHighF,
    };
  ...
  async _executeExit(trade, price, currentProbability, forecast, exitReason = 'evaluator', obs = null) {
    ...
    const exitData = {
      ...
      observation_high: trade.range_unit === 'C' ? obsHighC : obsHighF,
      wu_high: trade.range_unit === 'C' ? obsWuC : obsWuF,
    };
