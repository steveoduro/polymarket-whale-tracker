  ...
  async _evaluateSingle(trade) {
    ...
    if (shouldAct && (signalIsActive || globalActive)) {
      if (logEntry.exit_type === 'guaranteed_win') {
        // GW entries: already know they'll win — let resolver confirm with authoritative WU/NWS data
        // Edge/cal entries that became GW during monitoring: resolve immediately
        if (trade.entry_reason?.startsWith('guaranteed_win')) {
          this._log('info', `GW entry — deferring to resolver: ${trade.side} ${trade.city} ${trade.range_name}`);
        } else {
  ...
  async _resolveGuaranteed(trade, signal, currentProbability, forecast, obs) {
    ...
    const { error } = await query(
      ...
      [
        ...
        pnl,
        0,
        // Preserve PWS correctedMedian stored at entry — METAR running_high is meaningless for PWS trades
        trade.entry_reason === 'guaranteed_win_pws'
          ? trade.observation_high
