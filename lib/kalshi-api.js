...
class KalshiAPI {
  ...
  /**
   * Get all markets for an event (public endpoint, no auth needed)
   * Used by Bot B to fetch current prices for Kalshi positions
   * @param {string} eventTicker - e.g., 'KXHIGHAUS-26FEB11'
   */
  async getEventMarkets(eventTicker) {
    try {
      const data = await this.request(`/events/${eventTicker}`);
  ...
  parseMarketToRange(market) {
    try {
      ...
      const price = yesAsk; // Use ask as price (what we'd pay to buy)
