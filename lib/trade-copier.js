 ...
 * Supports multiple target wallets with per-whale strategies.
 */
...
// =============================================================================
// WHALE STRATEGIES - Per-whale copy rules
// =============================================================================
// Global settings from .env (COPY_TRADE_SIZE, DAILY_LOSS_LIMIT, etc.) still apply.
// These per-whale rules add additional filters on top of global settings.

const WHALE_STRATEGIES = {
  // NoonienSoong: 92% win rate + 0% fees = copy everything
  '0x38cc1d1f95d12039324809d8bb6ca6da6cbef88e': {
    username: 'NoonienSoong',
    markets: ['weather'],
    minWhaleSize: 10,      // Copy trades >= $10
    minPrice: null,        // No price filter - copy all
    notes: '92% win rate, 0% fees, weather specialist',
  },

  // micro88so8z: 100% win rate = copy everything
  '0x2ec681d5cbf2ba6d1e8f0e87b2e6026b0bc438c8': {
    username: 'micro88so8z',
    markets: ['crypto'],
    minWhaleSize: 10,      // Copy trades >= $10
    minPrice: null,        // No price filter - copy all
    notes: '100% win rate, BTC specialist',
  },

  // HondaCivic: 67% win rate = only high conviction trades
  '0x08cf0b0fec3d42d9920bb0dfbc49fde635088cbc': {
    username: 'HondaCivic',
    markets: ['weather'],
    minWhaleSize: 50,      // Only copy trades >= $50 (higher conviction)
    minPrice: null,        // No price filter
    notes: '67% win rate, need larger trades for conviction',
  },

  // distinct-baguette: 3.15% fees eat profits on low confidence
  // Only copy Solana and XRP (better edge on these markets)
  '0xe00740bce98a594e26861838885ab310ec3b548c': {
    username: 'distinct-baguette',
    markets: ['solana', 'xrp'],  // Only these cryptos, not BTC/ETH
    minWhaleSize: 25,      // Copy trades >= $25
    minPrice: 0.90,        // Only copy when price >= 90¢ (high confidence)
    notes: '3.15% fees, only SOL/XRP at 90¢+',
  },
};

// Build TARGET_WALLETS array from strategies
const TARGET_WALLETS = Object.entries(WHALE_STRATEGIES).map(([address, strategy]) => ({
  address,
...
const MARKET_FILTERS = {
  ...
  // Solana only
  solana: [
    /solana\s+up\s+or\s+down/i,
    /sol\s+up\s+or\s+down/i,
  ],
  // XRP only
  xrp: [
    /xrp\s+up\s+or\s+down/i,
  ],
  // Weather markets
  weather: [
    /temperature.*high/i,
...
class TradeCopier {
  ...
  async initialize() {
    ...
    // Log per-whale strategies
    this.log('info', '=== Whale Strategies ===');
    for (const [address, strategy] of Object.entries(WHALE_STRATEGIES)) {
      const minPriceStr = strategy.minPrice ? `${(strategy.minPrice * 100).toFixed(0)}¢+` : 'any';
      this.log('info', `  ${strategy.username}: min $${strategy.minWhaleSize}, price ${minPriceStr}`, {
        markets: strategy.markets,
    ...
    this.log('info', 'Trade Copier initialized', {
      targets: this.targetWallets.length,
      paperMode: this.paperMode,
    });
  }
  ...
  async processTrade(trade, wallet) {
    ...
    // Get per-whale strategy
    const strategy = WHALE_STRATEGIES[wallet.address.toLowerCase()];
    const whaleSize = parseFloat(trade.size) || 0;
    const tradePrice = parseFloat(trade.price) || 0;

    // Check per-whale minimum size (overrides global MIN_WHALE_SIZE)
    if (strategy?.minWhaleSize && whaleSize < strategy.minWhaleSize) {
      this.stats.tradesSkipped++;
      if (whaleStats) whaleStats.tradesSkipped++;
      this.log('info', 'Skipping - whale trade below minimum for this trader', {
        whale: target.username,
        tradeSize: whaleSize,
        minRequired: strategy.minWhaleSize,
      });
      return { copied: false, reason: `Whale trade $${whaleSize.toFixed(2)} < min $${strategy.minWhaleSize}` };
    }

    // Check per-whale minimum price filter
    if (strategy?.minPrice && tradePrice < strategy.minPrice) {
      this.stats.tradesSkipped++;
      if (whaleStats) whaleStats.tradesSkipped++;
      this.log('info', 'Skipping - price below minimum for this trader', {
        whale: target.username,
        price: tradePrice,
        minPrice: strategy.minPrice,
        notes: strategy.notes,
      });
      return { copied: false, reason: `Price ${(tradePrice * 100).toFixed(0)}¢ < min ${(strategy.minPrice * 100).toFixed(0)}¢` };
    }

    this.stats.tradesMatched++;
    this.log('info', 'Trade passed whale strategy filters', {
      whale: target.username,
      size: whaleSize,
      price: `${(tradePrice * 100).toFixed(0)}¢`,
      strategy: strategy?.notes || 'default',
    });
    ...
    // Check global risk rules (from .env settings)
...
module.exports = { TradeCopier, TARGET_WALLETS, MARKET_FILTERS, WHALE_STRATEGIES };
