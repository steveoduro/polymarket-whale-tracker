...
const CITIES = {
  ...
  'minneapolis':  { lat: 44.9778, lon: -93.2650, tz: 'America/Chicago', unit: 'F', nwsStation: 'KMSP' },
};
...
const CITY_SOURCE_PRIORITY = {
  ...
  'boston': ['nws', 'openmeteo', 'weatherapi'],           // NWS primary (east coast)
  'phoenix': ['nws', 'openmeteo', 'weatherapi'],         // NWS primary (US city)
  'minneapolis': ['nws', 'openmeteo', 'weatherapi'],     // NWS primary (midwest)
...
class WeatherAPI {
  ...
  getCity(cityName) {
    ...
    // Partial match — require the input to contain a full city key (or vice versa)
    // but prefer longer matches to avoid "las" matching "dallas" before "las vegas"
    let bestMatch = null;
    let bestLen = 0;
  ...
  /**
   * Get running daily high from METAR airport observations.
   * Uses aviationweather.gov (free, no key, works globally).
   * @param {string} stationId - ICAO station ID (e.g., 'KLGA', 'CYYZ', 'EGLC')
   * @param {string} dateStr - Date in YYYY-MM-DD format (local calendar date)
   * @param {string} timezone - IANA timezone (e.g., 'America/New_York')
   * @returns {Object|null} { highF, highC, date, stationId, source, observationCount }
   */
  async getMETARHigh(stationId, dateStr, timezone) {
    try {
      const url = `https://aviationweather.gov/api/data/metar?ids=${stationId}&format=json&hours=24`;

      const resp = await fetch(url);
      this.requestCount++;

      if (!resp.ok) {
        this.log('warn', `METAR API returned ${resp.status}`, { stationId, date: dateStr });
        return null;
      }
      ...
      if (!Array.isArray(data) || data.length === 0) {
        this.log('info', 'METAR returned no observations', { stationId, date: dateStr });
        return null;
      }

      // Get UTC offset for this date in this timezone (same logic as getNWSObservationHigh)
      const utcDate = new Date(`${dateStr}T12:00:00Z`);
      const formatter = new Intl.DateTimeFormat('en-US', {
        timeZone: timezone,
      ...
      // Local midnight boundaries in UTC
      const startUTC = new Date(`${dateStr}T00:00:00Z`);
      startUTC.setUTCHours(startUTC.getUTCHours() - offsetHours);
      const endUTC = new Date(startUTC.getTime() + 24 * 60 * 60 * 1000);

      // Filter observations to local calendar date and find max temp
      let maxC = -Infinity;
      let validCount = 0;
      ...
      if (validCount === 0 || maxC === -Infinity) {
        this.log('info', 'METAR: no valid observations for date', { stationId, date: dateStr, totalObs: data.length });
        return null;
      }
      ...
      this.log('info', 'METAR observation high', {
        stationId, date: dateStr,
        highF, highC,
        observationCount: validCount,
      });
      ...
      this.log('warn', 'METAR fetch failed', { stationId, date: dateStr, error: err.message });
      return null;
    }
  }

  /**
   * Get the running daily high temperature for a city on today's date.
   * Uses NWS observations for US cities, METAR for international.
   * Returns null before 10 AM local (high hasn't developed).
   * @param {string} city - City name
   * @returns {Object|null} { highF, highC, date, stationId, source, observationCount, localHour, highLikelyFinal }
   */
  async getCurrentDayHigh(city) {
    const cityConfig = CITIES[city.toLowerCase()];
    if (!cityConfig) return null;

    const tz = cityConfig.tz;
    const now = new Date();

    // Get local hour via toLocaleString re-parse (existing codebase pattern)
    const localNow = new Date(now.toLocaleString('en-US', { timeZone: tz }));
    const localHour = localNow.getHours();

    // Don't bother before 10 AM local — high hasn't developed
    if (localHour < 10) return null;

    // Get today's date string in local timezone (en-CA gives YYYY-MM-DD directly)
    const todayStr = now.toLocaleDateString('en-CA', { timeZone: tz });
    const highLikelyFinal = localHour >= 16;

    const station = cityConfig.nwsStation || cityConfig.polymarketStation;
    if (!station) return null;

    let result;
    if (cityConfig.nwsStation) {
      result = await this.getNWSObservationHigh(cityConfig.nwsStation, todayStr, tz);
    } else {
      result = await this.getMETARHigh(cityConfig.polymarketStation, todayStr, tz);
    }

    if (!result) return null;
    return { ...result, localHour, highLikelyFinal };
  }
  ...
  async getAllForecasts() {
    ...
    const uniqueCities = ['london', 'nyc', 'atlanta', 'miami', 'chicago',
                          ...
                          'houston', 'philadelphia', 'dc', 'las vegas',
                          'new orleans', 'san francisco', 'austin',
                          'boston', 'minneapolis'];
  ...
  async recordForecastAccuracy(supabase, city, marketDate, actualTempF) {
    ...
    try {
      ...
      // Calculate errors for each source (use != null to avoid treating 0°F as missing)
      const openMeteoError = forecast.high_temp_f != null
        ? Math.abs(parseFloat(forecast.high_temp_f) - actualTempF)
        : null;
      const nwsError = forecast.nws_high_f != null
        ? Math.abs(parseFloat(forecast.nws_high_f) - actualTempF)
        : null;
      const weatherApiError = forecast.weatherapi_high_f != null
      ...
      const accuracy = {
        ...
        open_meteo_forecast_f: forecast.high_temp_f != null ? parseFloat(forecast.high_temp_f) : null,
        open_meteo_error_f: openMeteoError,
        // NWS
        nws_forecast_f: forecast.nws_high_f != null ? parseFloat(forecast.nws_high_f) : null,
        nws_error_f: nwsError,
        // WeatherAPI
        weatherapi_forecast_f: forecast.weatherapi_high_f != null ? parseFloat(forecast.weatherapi_high_f) : null,
        weatherapi_error_f: weatherApiError,
        // Actual and best source
        actual_temp_f: actualTempF,
        best_source: bestSource,
        // Consensus
        consensus_forecast_f: consensusForecast != null ? Math.round(consensusForecast * 10) / 10 : null,
        consensus_error_f: consensusError != null ? Math.round(consensusError * 10) / 10 : null,
        // Keep deprecated columns
        tomorrow_forecast_f: forecast.tomorrow_high_f != null ? parseFloat(forecast.tomorrow_high_f) : null,
        tomorrow_error_f: forecast.tomorrow_high_f != null
