  ...
  async recordForecastAccuracy(supabase, city, marketDate, actualTempF) {
    ...
    try {
      ...
      const accuracy = {
        ...
        open_meteo_forecast_f: parseFloat(forecast.high_temp_f),
        tomorrow_forecast_f: forecast.tomorrow_high_f ? parseFloat(forecast.tomorrow_high_f) : null,
        actual_temp_f: actualTempF,
        open_meteo_error_f: Math.abs(parseFloat(forecast.high_temp_f) - actualTempF),
      ...
      this.log('info', 'Recorded forecast accuracy', {
        city,
        date: marketDate,
        openMeteoError: accuracy.open_meteo_error_f.toFixed(1) + '°F',
        tomorrowError: accuracy.tomorrow_error_f?.toFixed(1) + '°F' || 'N/A',
      });
