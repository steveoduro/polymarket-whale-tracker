...
class METARObserver {
  ...
  async metarFastPoll() {
        ...
        for (const range of ranges) {
          ...
          try {
            ...
            const otherRange = ranges.find(r => {
              ...
              // °C city: convert range boundaries for cross-platform match (tolerance < 1°F for rounding)
    ...
    const durationSec = ((Date.now() - fastPollStart) / 1000).toFixed(1);
    if (detections > 0 || Number(durationSec) > 15) {
      this._log('info', `Fast poll complete: ${polled} cities, ${detections} detections, ${durationSec}s`);
    }
