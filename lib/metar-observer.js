...
class METARObserver {
   ...
   */
  async _processRangesForCity(cityKey, cityConfig, localToday, effHighC, effHighF, ranges, eventMap, pollSource) {
  ...
  async metarFastPoll() {
    ...
    // 1. Build list of active cities and their stations
    const activeCities = []; // { cityKey, cityConfig, localToday, station }
    ...
    for (const [cityKey, cityConfig] of Object.entries(config.cities)) {
      ...
      activeCities.push({ cityKey, cityConfig, localToday, station });
    ...
    const cityDatePairs = activeCities
      .filter(c => metarMap.has(c.station))
      .map(c => ({ cityKey: c.cityKey, localToday: c.localToday }));
    const uniqueDates = [...new Set(cityDatePairs.map(p => p.localToday))];

    // 3a. Batch running highs: one query using DISTINCT ON
    const runningHighMap = new Map(); // "city|date" → { running_high_c, running_high_f }
    try {
      const { data: highRows } = await query(
        `SELECT DISTINCT ON (city, target_date) city, target_date, running_high_c, running_high_f
         FROM metar_observations
         WHERE target_date = ANY($1)
         ORDER BY city, target_date, created_at DESC`,
        [uniqueDates]
      );
      for (const row of (highRows || [])) {
        runningHighMap.set(`${row.city}|${row.target_date}`, row);
      }
    } catch (err) {
    ...
    for (const { cityKey, cityConfig, localToday, station } of activeCities) {
      ...
      try {
        ...
        // Look up running high from batch result
        const highRow = runningHighMap.get(`${cityKey}|${localToday}`);
        ...
        // Get market ranges (cached by adapter)
