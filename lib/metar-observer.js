...
class METARObserver {
  constructor(alerts, platformAdapter) {
    ...
    this._pendingAlerted = new Set(); // debounce: city_date_rangeName — reset daily
   ...
   * Fires a pending alert so the user knows a guaranteed_win is imminent.
   */
    ...
    try {
      ...
      for (const range of ranges) {
        ...
        if (range.rangeMin != null && range.rangeMax != null && metarHigh > range.rangeMax) {
          if (wuHigh != null && wuHigh > range.rangeMax) continue; // Both crossed — not pending
          metarCrossed = true;
        }

        if (!metarCrossed) continue;

        // Debounce: only alert once per city+date+range
        const dedupeKey = `${cityKey}_${targetDate}_${range.rangeName}`;
        if (this._pendingAlerted.has(dedupeKey)) continue;
        this._pendingAlerted.add(dedupeKey);

        rangesAffected.push(range.rangeName);
      }

      if (rangesAffected.length > 0) {
