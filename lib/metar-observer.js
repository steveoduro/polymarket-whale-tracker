...
class METARObserver {
   ...
   * Called on its own setInterval (90s default), not inside _runCycle().
   */
  async metarFastPoll() {
    const gwConfig = config.guaranteed_entry;
    if (!gwConfig?.ENABLED) return { polled: 0, detections: 0 };

    const minGapF = gwConfig.METAR_ONLY_MIN_GAP_F || 2.0;
    const minGapC = gwConfig.METAR_ONLY_MIN_GAP_C || 1.0;
    const { start: activeStart, end: activeEnd } = config.observer.ACTIVE_HOURS;
    ...
    for (const [cityKey, cityConfig] of Object.entries(config.cities)) {
      ...
      try {
        // 1. Fetch latest METAR (lightweight — no DB write)
        const url = `${METAR_API_BASE}?ids=${station}&format=json`;
        const resp = await this._fetch(url);
        if (!resp.ok) continue;
        ...
        const latest = data[0];
        if (latest.temp == null) continue;

        const tempC = latest.temp;
        const tempF = Math.round(tempC * 9 / 5 + 32);
        polled++;

        // 2. Get DB running high (last observation from main observer)
        const { data: dbRows } = await query(
          `SELECT running_high_c, running_high_f
           FROM metar_observations
           WHERE city = $1 AND target_date = $2
           ORDER BY created_at DESC LIMIT 1`,
          [cityKey, localToday]
        );
        const dbHighC = dbRows?.[0]?.running_high_c ?? -Infinity;
        const dbHighF = dbRows?.[0]?.running_high_f ?? -Infinity;

        // Effective high = max(current METAR, DB running high)
        ...
        const minGap = unit === 'C' ? minGapC : minGapF;

        // 3. Get market ranges
        const ranges = await this.adapter.getMarkets(cityKey, localToday);
        if (!ranges || ranges.length === 0) continue;

        // 4. Batch-load existing pending events for UPDATE checks
        const { data: existing } = await query(
          `SELECT id, platform, range_name, side, wu_confirmed_at, market_repriced_at,
                  kalshi_market_repriced_at
           FROM metar_pending_events WHERE city = $1 AND target_date = $2`,
          [cityKey, localToday]
        );
        const eventMap = new Map();
        ...
        // 5. Check each range for boundary crossing
        for (const range of ranges) {
          if (config.platforms[range.platform]?.tradingEnabled === false) continue;
          ...
          if (!side) continue;
          if (gap < minGap) continue; // Below gap threshold

          const winningAsk = side === 'YES' ? range.ask : (1 - range.bid);
          const evtKey = `${range.platform}_${range.rangeName}_${side}`;
          const existingEvt = eventMap.get(evtKey);

          // 6. Snapshot orderbook for this range (fresh, bypass cache for Kalshi)
          let askDepth = null;
          ...
          try {
            if (range.platform === 'polymarket' && range.tokenId) {
              ...
              this._log('warn', `Fast poll: no tokenId for ${range.rangeName} (Polymarket)`, { city: cityKey });
            } else if (range.platform === 'kalshi' && range.marketId) {
              // Bypass cache for fresh snapshot
              ...
              this._log('warn', `Fast poll: no marketId for ${range.rangeName} (Kalshi)`, { city: cityKey });
            }

            // Cross-platform: also snapshot the OTHER platform if available
            // Find matching range on the other platform
            // Polymarket uses city unit (°C or °F), Kalshi always uses °F
            ...
            const otherRange = ranges.find(r => {
              ...
              // Same-unit platforms: direct comparison
              if (unit === 'F') return r.rangeMin === range.rangeMin && r.rangeMax === range.rangeMax;
              // °C city: convert range boundaries for cross-platform match
              if (range.platform === 'polymarket') {
                // Polymarket °C → Kalshi °F: convert our °C to °F for comparison
                const minF = range.rangeMin != null ? Math.round(range.rangeMin * 9 / 5 + 32) : null;
                const maxF = range.rangeMax != null ? Math.round(range.rangeMax * 9 / 5 + 32) : null;
                return r.rangeMin === minF && r.rangeMax === maxF;
              } else {
                // Kalshi °F → Polymarket °C: convert our °F to °C for comparison
                const minC = range.rangeMin != null ? Math.round((range.rangeMin - 32) * 5 / 9) : null;
                const maxC = range.rangeMax != null ? Math.round((range.rangeMax - 32) * 5 / 9) : null;
                return r.rangeMin === minC && r.rangeMax === maxC;
              }
            });
            if (otherRange) {
            ...
            this._log('warn', `Fast poll orderbook snapshot failed`, { city: cityKey, range: range.rangeName, error: err.message });
          }
          ...
          if (!existingEvt) {
            ...
            // Alert (debounce by in-memory Set with fast_ prefix)
            ...
            if (!this._fastPollAlerted.has(alertKey)) {
              ...
              this._log('info', `Fast poll detection: ${cityKey} ${range.rangeName} ${side}`, {
            ...
            // WU confirmation: skip in fast poll (no WU data)
    ...
    if (detections > 0) {
      this._log('info', `Fast poll complete: ${polled} cities, ${detections} new detections`);
    }
