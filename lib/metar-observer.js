...
class METARObserver {
   ...
   * Cached for 30 min. Uses 2-day window, filtered to GW hours (10am-4pm local)
   * per city timezone — all-day averages mask midday heat-spike inaccuracy.
   */
  async _loadPwsAvgErrorCache() {
    try {
      // Build per-city timezone VALUES for the query
      const cityTzPairs = Object.entries(config.cities)
        .filter(([, c]) => c.tz)
        .map(([k, c]) => `('${k}', '${c.tz}')`);
      if (cityTzPairs.length === 0) return;

      const { data: rows } = await query(
        `WITH city_tz(city, tz) AS (VALUES ${cityTzPairs.join(', ')})
         SELECT p.city, AVG(ABS(pws_corrected_median - metar_temp)) as avg_error
         FROM pws_observations p
         JOIN city_tz ct ON ct.city = p.city
         WHERE polled_at >= NOW() - INTERVAL '2 days'
           AND pws_corrected_median IS NOT NULL AND metar_temp IS NOT NULL
           AND stations_online >= 2
           AND EXTRACT(HOUR FROM polled_at AT TIME ZONE ct.tz) BETWEEN 10 AND 15
         GROUP BY p.city`
      );
  ...
  async _fetchPwsBatch(activeCities, metarMap) {
    ...
    for (const [cityKey, stationResults] of cityResults) {
      ...
      // Bias-corrected median (true median, not weighted average)
      ...
      for (const s of stationResults) {
        ...
        if (bias && bias.n_samples >= BIAS_MIN_SAMPLES && bias.rolling_bias != null && bias.reliable) {
          // Calibrated: apply bias correction
          correctedTemp = Math.round((s.temp - bias.rolling_bias) * 10) / 10;
        } else {
          // Warmup (no bias entry or < 576 samples): use raw temp
          correctedTemp = s.temp;
        }
        corrTemps.push(correctedTemp);
      }

      if (corrTemps.length > 0) {
        corrTemps.sort((a, b) => a - b);
        const mid = Math.floor(corrTemps.length / 2);
        correctedMedian = corrTemps.length % 2 === 1
          ? corrTemps[mid]
          : Math.round(((corrTemps[mid - 1] + corrTemps[mid]) / 2) * 10) / 10;
