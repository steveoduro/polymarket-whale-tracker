...
class METARObserver {
  ...
  /**
   * Poll METAR observations for all cities with open positions.
   * Returns { citiesPolled, newHighs, observations }
   */
  async observe() {
    const stats = { citiesPolled: 0, newHighs: 0, observations: [] };

    // Get cities with open positions
    const { data: openTrades, error } = await db
      .from('trades')
      .select('city, target_date, platform')
      .eq('status', 'open');

    if (error || !openTrades || openTrades.length === 0) return stats;

    // Group by city → { stations[], targetDates[], tz }
    ...
    for (const trade of openTrades) {
      ...
      // Add the station for this platform
      ...
      for (const stationId of info.stations) {
        try {
          ...
          if (obs) {
            ...
            // WU cross-validation: poll WU for same city/date
            const wuObs = await this._pollWU(cityKey, localToday);
            if (wuObs) {
              // Update the METAR observation row with WU data
              await this._updateWUData(cityKey, localToday, obs.observedAt, wuObs);

              // Use higher of WU/METAR for running high (conservative for loss detection)
              const metarHigh = info.unit === 'C' ? obs.runningHighC : obs.runningHighF;
              const wuHigh = info.unit === 'C' ? wuObs.highC : wuObs.highF;
              if (wuHigh > metarHigh) {
                // WU captured a higher reading — update running high
                const newHighF = Math.max(obs.runningHighF, wuObs.highF);
                const newHighC = Math.max(obs.runningHighC, wuObs.highC);
                await db.from('metar_observations')
                  .update({ running_high_f: newHighF, running_high_c: newHighC })
                  .eq('city', cityKey)
                  .eq('target_date', localToday)
                  .eq('observed_at', obs.observedAt);
                this._log('info', `WU higher than METAR: ${cityKey}`, {
                  metar: metarHigh, wu: wuHigh, updated: info.unit === 'C' ? newHighC : newHighF,
                });
              }
              if (metarHigh !== wuHigh) {
