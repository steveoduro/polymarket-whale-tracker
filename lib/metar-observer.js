...
class METARObserver {
   ...
   * @param {string} pollSource - 'fast_poll' or 'websocket'
  ...
  async metarFastPoll() {
    ...
    // 3. Process each active city using batch results
    let polled = 0;
    let detections = 0;
    ...
    for (const { cityKey, cityConfig, localToday, station } of activeCities) {
      ...
      try {
        ...
        // Get DB running high (last observation from main observer)
        ...
        // Batch-load existing pending events for UPDATE checks
        const { data: existing } = await query(
