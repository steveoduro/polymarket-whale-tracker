...
class METARObserver {
  ...
  async metarFastPoll() {
    ...
    let pwsMap = new Map();
    try {
      pwsMap = await this._fetchPwsBatch(activeCities);
    } catch (err) {
   ...
   */
  async _fetchPwsBatch(activeCities) {
    ...
    const results = await Promise.allSettled(
      fetchList.map(async ({ cityKey, stationId, units, unit }) => {
        ...
        try {
          ...
          return { cityKey, stationId, unit, temp, ageSec, epoch };
        } catch (err) {
    ...
    const cityResults = new Map(); // cityKey → [{ stationId, temp, ageSec, online }]
    ...
    for (const [cityKey, stationResults] of cityResults) {
      ...
      pwsMap.set(cityKey, {
        stationIds, temps, ages, online: onlineArr,
