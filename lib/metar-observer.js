...
class METARObserver {
   ...
   */
  async _processRangesForCity(cityKey, cityConfig, localToday, effHighC, effHighF, ranges, eventMap, pollSource, detectionStation, platformFilter) {
    ...
    for (const range of ranges) {
      ...
      // Platform-aware min gap: Kalshi needs wider buffer (NWS CLI vs METAR divergence)
      const isKalshi = range.platform === 'kalshi';
      const minGap = unit === 'C'
        ? (isKalshi ? (gwConfig.METAR_ONLY_MIN_GAP_C_KALSHI || 0.8) : (gwConfig.METAR_ONLY_MIN_GAP_C || 0.5))
        : (isKalshi ? (gwConfig.METAR_ONLY_MIN_GAP_F_KALSHI || 1.5) : (gwConfig.METAR_ONLY_MIN_GAP_F || 0.5));
      ...
      if (!existingEvt) {
        ...
        await query(
          ...
          [cityKey, localToday, range.platform, range.rangeName, side,
           ...
           pollSource, detectionStation || null]
        );
        detections++;
  ...
  async metarFastPoll() {
    ...
    // 4. Process each active city using batch results
    let polled = 0;
    let detections = 0;

    for (const { cityKey, cityConfig, localToday, stationsByPlatform } of activeCities) {
      // Check if ANY station has METAR data
      ...
      try {
        ...
        // Get market ranges (cached by adapter) — full set for all platforms
        const ranges = await this.adapter.getMarkets(cityKey, localToday);
        if (!ranges || ranges.length === 0) continue;

        // Look up pending events from batch result
        const eventMap = pendingEventMap.get(`${cityKey}|${localToday}`) || new Map();

        // Build station→platforms[] groups (invert stationsByPlatform)
        // e.g. NYC: { KLGA: Set['polymarket'], KNYC: Set['kalshi'] }
        // e.g. Atlanta: { KATL: Set['polymarket', 'kalshi'] }
        ...
        // Process each station group with station-specific temps
        for (const [station, platforms] of stationGroups) {
          ...
          // Look up station-specific running high from batch result
          ...
          // Process ranges — full ranges passed for cross-platform orderbook lookup,
          // platformFilter restricts which ranges get detection processing
    ...
    if (detections > 0 && this.scanner && this.executor) {
      try {
        ...
        await this._writeObservationsFromFastPoll(activeCities, metarMap, runningHighMap);
   ...
   * Only writes when current temp exceeds stored running high (new high).
   * Uses GREATEST in upsert to never lower the running_high.
   */
  async _writeObservationsFromFastPoll(activeCities, metarMap, runningHighMap) {
    let writes = 0;
    for (const { cityKey, cityConfig, localToday, stationsByPlatform } of activeCities) {
      ...
      for (const station of uniqueStations) {
        ...
        // Check against stored running high — only write if new high
        const highKey = `${cityKey}|${localToday}|${station}`;
        const highRow = runningHighMap.get(highKey);
        const dbHighC = highRow?.running_high_c ?? -Infinity;
        const dbHighF = highRow?.running_high_f ?? -Infinity;

        if (dbHighC !== -Infinity && tempC <= dbHighC) continue;

        const runHighC = Math.max(tempC, dbHighC === -Infinity ? tempC : dbHighC);
        const runHighF = Math.max(tempF, dbHighF === -Infinity ? tempF : dbHighF);
        ...
        try {
          await query(
            ...
            [cityKey, station, localToday, observedAt.toISOString(), tempC, tempF, runHighC, runHighF]
          );
          writes++;
        } catch (err) {
