...
class Alerts {
  ...
  async guaranteedWinMissed(entries) {
    for (const entry of entries) {
      ...
      const isPws = entry.reason?.startsWith('below_pws') || entry.isPws;
      const sources = [];
      if (isPws && entry.observation_high != null) {
        sources.push(`PWS: ${entry.observation_high}°${unit}`);
      } else {
      ...
      } else if (entry.reason === 'below_pws_gap') {
        reasonText = `PWS gap ${entry.gap}° below min ${entry.minGap}°`;
      } else {
        reasonText = entry.reason;
      }

      const missedEmoji = isPws ? '📡' : '💡';
      const lines = [
        `${missedEmoji} ${isPws ? 'PWS' : 'METAR'} GW MISSED`,
        ``,
        `📍 ${entry.city.toUpperCase()} — ${entry.target_date}`,
