...
class Alerts {
  ...
  tradeEntry(trade) {
    ...
    const emoji = isGW ? '🎯' : isCal ? '📐' : (side === 'NO' ? '🔴' : '🟢');
    const tag = isGW ? ' GW' : isCal ? ' CAL' : '';
    const unit = trade.range_unit || 'F';
    const lines = [
      `${emoji} NEW ${side}${tag} TRADE [${trade.platform.toUpperCase()}]`,
      ``,
  ...
  async guaranteedWinDetected(entries) {
    for (const entry of entries) {
      ...
      const header = isDual ? '🎯 GUARANTEED WIN DETECTED' : '⚠️ GUARANTEED WIN — SINGLE SOURCE';
      const dualFlag = isDual ? 'DUAL' : 'SINGLE';
      const sources = [];
      if (entry.observation_high != null) sources.push(`METAR: ${entry.observation_high}°${unit}`);
      if (entry.wu_high != null) sources.push(`WU: ${entry.wu_high}°${unit}`);
      const lines = [
        header,
        ``,
        ...
        `🔍 ${dualFlag} confirmed — ${sources.join(', ')}`,
        ``,
        `Executing...`,
      ];
  ...
  tradeExit(trade) {
    ...
    const tag = isGW ? ' GW' : isCal ? ' CAL' : '';
    const lines = [
      `${pnlEmoji}${tag} EXIT [${trade.platform.toUpperCase()}] — ${trade.exit_reason}`,
      ``,
      `📍 ${trade.city.toUpperCase()} — ${trade.target_date}`,
  ...
  tradeResolved(trade) {
    ...
    const unit = trade.range_unit || 'F';
    const lines = [
      `${emoji}${tag} RESOLVED [${trade.platform.toUpperCase()}]`,
      ``,
      `📍 ${trade.city.toUpperCase()} — ${trade.target_date}`,
  ...
  async metarPending(data) {
    ...
    const lines = [
      `⏳ METAR BOUNDARY CROSSED`,
      ``,
      `📍 ${data.city.toUpperCase()} — ${data.date}`,
      `🌡️ METAR: ${data.metarHigh}°${unit} | WU: ${wuText}`,
