...
class Alerts {
  ...
  async guaranteedWinDetected(entries) {
    for (const entry of entries) {
      ...
      let confirmLine;
      if (wuTriggered && isDual) {
        confirmLine = `🔍 WU-triggered, METAR confirmed — ${[wuText, metarText].filter(Boolean).join(', ')}`;
      } else if (wuTriggered) {
        confirmLine = `🔍 WU-triggered, METAR pending — ${[wuText, metarText].filter(Boolean).join(', ')}`;
      } else if (isDual) {
        confirmLine = `🔍 METAR-triggered, WU confirmed — ${[metarText, wuText].filter(Boolean).join(', ')}`;
      } else {
        confirmLine = `🔍 SINGLE confirmed — ${[metarText, wuText].filter(Boolean).join(', ')}`;
      }
