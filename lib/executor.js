...
class Executor {
  ...
  async _executeSingle(opp) {
    ...
    if (side === 'YES') {
      const { data: existingYes, error: yesError } = await db
        .from('trades')
        .select('id, range_name')
        .eq('city', opp.city)
        .eq('target_date', opp.target_date)
        .eq('side', 'YES')
        .eq('status', 'open')
    ...
    if (side === 'NO') {
      const { data: existingNo, error: noError } = await db
        .from('trades')
        .select('id, range_name')
        .eq('city', opp.city)
        .eq('target_date', opp.target_date)
  ...
  async _executeGuaranteedSingle(entry) {
    ...
    const { data: existing, error: dupError } = await db
      ...
      .select('id')
      .eq('city', entry.city)
      .eq('target_date', entry.target_date)
      .eq('range_name', entry.range_name)
      .eq('side', side)
  ...
  _calculateSize(opp, bankroll) {
    ...
    // Kelly: f* = (b*p - q) / b where b = netProfit/effectiveCost (true net odds)
