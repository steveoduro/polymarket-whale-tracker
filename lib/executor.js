...
class Executor {
  constructor(platformAdapter, alerts) {
    ...
    // Split GW bankrolls: live (real money) vs paper (simulation)
    this.gwLiveBankroll = config.guaranteed_entry.GW_METAR_BANKROLL || 0;
    this.gwPaperBankroll = config.guaranteed_entry.GW_PAPER_BANKROLL || 1000;
    this.pwsGwBankroll = config.pws_gw?.BANKROLL || 500;
    this.bankrollValid = false;

    // Track per-date NO exposure: { 'YYYY-MM-DD': dollars }
    this.noExposureByDate = {};

    // In-memory GW dedup — closes race window between fast-poll cycles
    this._gwSubmitted = new Set();   // "city:date:range:side:platform"
    this._gwSubmittedDate = null;    // reset daily
  }
  ...
  async initBankrolls() {
    try {
      const { data: openTrades, error } = await query(
        'SELECT side, cost, target_date, entry_reason, execution_mode FROM trades WHERE status = $1',
        ['open']
      );
      ...
      let yesDeployed = 0;
      let noDeployed = 0;
      let gwLiveDeployed = 0;
      let gwPaperDeployed = 0;
      let pwsGwDeployed = 0;

      for (const t of openTrades || []) {
        const cost = Number(t.cost) || 0;
        if (t.entry_reason === 'guaranteed_win_pws') {
          pwsGwDeployed += cost;
        } else if (t.entry_reason === 'guaranteed_win_metar_only') {
          if (t.execution_mode === 'live') {
            gwLiveDeployed += cost;
          } else {
            gwPaperDeployed += cost;
          }
        } else if (t.side === 'YES') {
          yesDeployed += cost;
        } else {
      ...
      this.gwLiveBankroll = (config.guaranteed_entry.GW_METAR_BANKROLL || 0) - gwLiveDeployed;
      this.gwPaperBankroll = (config.guaranteed_entry.GW_PAPER_BANKROLL || 1000) - gwPaperDeployed;
      this.pwsGwBankroll = (config.pws_gw?.BANKROLL || 500) - pwsGwDeployed;

      // NaN guard: validate bankrolls before allowing any trades
      if (!Number.isFinite(this.yesBankroll) || !Number.isFinite(this.noBankroll)
          || !Number.isFinite(this.gwLiveBankroll) || !Number.isFinite(this.gwPaperBankroll)
          || !Number.isFinite(this.pwsGwBankroll)) {
        this._log('error', 'Bankroll computed as NaN/Infinity — blocking all trades', {
          yesBankroll: this.yesBankroll,
          noBankroll: this.noBankroll,
          gwLiveBankroll: this.gwLiveBankroll,
          gwPaperBankroll: this.gwPaperBankroll,
          pwsGwBankroll: this.pwsGwBankroll,
          yesDeployed,
          noDeployed,
          gwLiveDeployed,
          gwPaperDeployed,
        ...
        this.yesBankroll = 0;
        this.noBankroll = 0;
        this.gwLiveBankroll = 0;
        this.gwPaperBankroll = 0;
        this.pwsGwBankroll = 0;
        this.bankrollValid = false;
        return;
      }

      this.bankrollValid = true;

      this._log('info', 'Bankrolls initialized', {
        yesBankroll: this.yesBankroll.toFixed(2),
        noBankroll: this.noBankroll.toFixed(2),
        gwLiveBankroll: this.gwLiveBankroll.toFixed(2),
        gwPaperBankroll: this.gwPaperBankroll.toFixed(2),
        pwsGwBankroll: this.pwsGwBankroll.toFixed(2),
  ...
  async _executeSingle(opp) {
    if (!this.bankrollValid) {
      this._log('warn', 'Skipping execution — bankroll invalid (NaN)');
      return null;
    }
    ...
    if (!Number.isFinite(bankroll)) {
      this._log('error', `${side} bankroll is NaN — aborting trade`);
      return null;
    }
    ...
    const { data, error } = await queryOne(
      ...
      [
        ...
        'paper', null, // execution_mode, order_id — regular engine is always paper
        tradeRecord.status,
  ...
  async _executeGuaranteedSingle(entry) {
    if (!this.bankrollValid) {
      this._log('warn', 'Skipping guaranteed execution — bankroll invalid (NaN)');
      return null;
    }

    const side = entry.side;
    const isPws = entry.entry_reason === 'guaranteed_win_pws';
    const isMetarOnly = entry.entry_reason === 'guaranteed_win_metar_only';

    // Determine live eligibility: GW_LIVE_ENABLED + polymarket + has tokenId (never for PWS — paper only for now)
    const isLive = !isPws && config.guaranteed_entry.GW_LIVE_ENABLED
      && entry.platform === 'polymarket'
      && entry.token_id;

    // Select matching bankroll
    let bankroll, bankrollLabel;
    if (isPws) {
      bankroll = this.pwsGwBankroll;
      bankrollLabel = 'pwsGw';
    } else if (isMetarOnly) {
      bankroll = isLive ? this.gwLiveBankroll : this.gwPaperBankroll;
      bankrollLabel = isLive ? 'gwLive' : 'gwPaper';
    } else {
      bankroll = side === 'YES' ? this.yesBankroll : this.noBankroll;
      bankrollLabel = side;
    }

    if (!Number.isFinite(bankroll)) {
      this._log('error', `${bankrollLabel} bankroll is NaN — aborting guaranteed trade`);
      return null;
    }

    // Check bankroll
    if (bankroll <= config.sizing.MIN_BET) {
      this._log('warn', `Skipping guaranteed ${entry.city} ${entry.range_name} ${side}: ${bankrollLabel} bankroll depleted ($${bankroll.toFixed(2)})`);
      return null;
    }
    ...
    // In-memory race dedup — synchronous, closes window between async DB checks
    const today = new Date().toISOString().split('T')[0];
    if (this._gwSubmittedDate !== today) {
      this._gwSubmitted.clear();
      this._gwSubmittedDate = today;
    }
    // Dedup key includes entry_reason so PWS and METAR can both enter the same market
    const entryReason = entry.entry_reason || 'guaranteed_win';
    const submitKey = `${entryReason}:${entry.city}:${entry.target_date}:${entry.range_name}:${side}:${entry.platform}`;
    if (this._gwSubmitted.has(submitKey)) {
      this._log('info', `GW race dedup: already submitted ${entryReason} ${entry.city} ${entry.range_name} ${side}`);
      return null;
    }
    this._gwSubmitted.add(submitKey);

    // Duplicate check against DB — filter by entry_reason group so PWS and METAR are independent
    let dupQuery, dupParams;
    if (isPws) {
      dupQuery = `SELECT id FROM trades
         WHERE city = $1 AND target_date = $2 AND range_name = $3
           AND side = $4 AND platform = $5 AND status IN ($6, $7)
           AND entry_reason = 'guaranteed_win_pws'
         LIMIT 1`;
      dupParams = [entry.city, entry.target_date, entry.range_name, side, entry.platform, 'open', 'resolved'];
    } else {
      dupQuery = `SELECT id FROM trades
         WHERE city = $1 AND target_date = $2 AND range_name = $3
           AND side = $4 AND platform = $5 AND status IN ($6, $7)
           AND entry_reason IN ('guaranteed_win', 'guaranteed_win_metar_only')
         LIMIT 1`;
      dupParams = [entry.city, entry.target_date, entry.range_name, side, entry.platform, 'open', 'resolved'];
    }

    const { data: existing, error: dupError } = await query(dupQuery, dupParams);
    ...
    // Mutual exclusivity: max 1 YES per city/date/platform (only one range can be YES)
    if (side === 'YES') {
      const { data: existingYes, error: yesError } = await query(
        `SELECT id, range_name FROM trades
         WHERE city = $1 AND target_date = $2 AND side = $3 AND platform = $4 AND status = $5
         LIMIT 1`,
        [entry.city, entry.target_date, 'YES', entry.platform, 'open']
      );
    ...
    // Adjacent-NO protection: don't enter NO when its range_max is at or below an open YES's range_min
    // (correlated risk — if YES wins, this NO was never in danger; if YES loses, this NO likely loses too)
    if (side === 'NO' && entry.range_max != null) {
      const { data: openYesTrades, error: yesErr } = await query(
        `SELECT range_min FROM trades
         WHERE city = $1 AND target_date = $2 AND side = 'YES' AND platform = $3 AND status = 'open'
         LIMIT 1`,
        [entry.city, entry.target_date, entry.platform]
      );
      if (!yesErr && openYesTrades && openYesTrades.length > 0) {
        const yesRangeMin = openYesTrades[0].range_min;
        if (yesRangeMin != null && entry.range_max <= yesRangeMin) {
          this._log('info', `Skipping adjacent NO ${entry.city} ${entry.range_name}: range_max ${entry.range_max} <= YES range_min ${yesRangeMin}`);
          return null;
        }
      }
    }

    // Sizing: PWS uses confidence-weighted, METAR uses fixed % of bankroll
    ...
    if (isPws) {
      ...
      const pwsCfg = config.pws_gw;
      const minConf = pwsCfg.MIN_CONFIDENCE_FACTOR || 0.3;
      const maxError = pwsCfg.MAX_AVG_CORRECTED_ERROR || 2.0;

      // City confidence: lower corrected error → higher factor
      const avgError = entry.pws_avg_error || maxError;
      const cityConf = Math.max(minConf, Math.min(1.0, (maxError - avgError) / maxError));

      // Time factor: earlier local hour → higher factor (morning=1.0, afternoon=low)
      const cityTz = config.cities[entry.city]?.tz;
      let timeConf = 1.0;
      if (cityTz) {
        const localHour = parseInt(new Intl.DateTimeFormat('en-US', {
          timeZone: cityTz, hour: 'numeric', hour12: false,
        }).format(new Date()), 10);
        const fullHour = pwsCfg.TIME_FULL_CONFIDENCE_HOUR || 12;
        const reducedHour = pwsCfg.TIME_REDUCED_HOUR || 15;
        if (localHour <= fullHour) {
          timeConf = 1.0;
        } else if (localHour >= reducedHour) {
          timeConf = minConf;
        } else {
          timeConf = Math.max(minConf, 1.0 - (localHour - fullHour) / (reducedHour - fullHour) * (1.0 - minConf));
        }
      }

      const confidence = cityConf * timeConf;
      const maxPct = pwsCfg.MAX_BANKROLL_PCT || 0.15;
      dollars = Math.min(bankroll * maxPct * confidence, bankroll);

      this._log('info', `PWS sizing: city=${entry.city} avgErr=${avgError.toFixed(2)} cityConf=${cityConf.toFixed(2)} timeConf=${timeConf.toFixed(2)} conf=${confidence.toFixed(2)} bankroll=$${bankroll.toFixed(2)} → $${dollars.toFixed(2)}`);
    } else {
      const maxPct = config.guaranteed_entry.MAX_BANKROLL_PCT;
      dollars = Math.min(bankroll * maxPct, bankroll);
    }
    ...
    // Set live flag on entry so executeBuy knows
    entry.live = isLive;
    entry.shares = shares;

    // Pass pre-fetched CLOB book ask to skip Gamma API staleness check (fast-path GW)
    if (entry._freshBookAsk != null) {
      entry._freshAskFromBook = entry._freshBookAsk;
    }

    // Execute (paper or live)
    const execution = await this.adapter.executeBuy(entry);

    // Handle aborted live orders (price moved up)
    if (execution.aborted) {
      this._log('warn', `GW live order aborted: ${execution.reason}`, {
        city: entry.city, range: entry.range_name,
        detectedAsk: execution.detectedAsk, currentAsk: execution.currentAsk,
      });
      return null;
    }
    ...
    if (!execution.paper) {
      // Live order placed — poll for fill
      ...
      for (let i = 0; i < 15; i++) {
        ...
        try {
          ...
          if (status.status === 'MATCHED' || (status.size_matched && status.size_matched >= shares)) {
            filled = true;
            break;
          }
          if (status.status === 'CANCELED') break;
        } catch (err) {
          this._log('warn', `Fill check error (attempt ${i + 1}/15)`, { error: err.message });
        }
      }
      ...
      if (!filled) {
        try {
          ...
          this._log('warn', 'Failed to cancel unfilled order', { orderId, error: err.message });
        }
        this._log('warn', 'GW live order not filled — cancelled', {
          orderId, city: entry.city, range: entry.range_name,
        });
        return null; // don't record trade
      }

      this._log('info', `GW live order FILLED`, { orderId, city: entry.city, shares });
    }

    // For live trades, use actual fill price (orderPrice) instead of detected ask
    const actualAsk = (!execution.paper && execution.orderPrice) ? execution.orderPrice : entry.ask;
    const actualCost = Math.round(shares * actualAsk * 100) / 100;
    ...
    const { data, error } = await queryOne(
      `INSERT INTO trades (
        ...
        pct_of_volume, execution_mode, order_id,
        status, current_probability, current_bid, current_ask,
        max_price_seen, min_probability_seen, evaluator_log
      ) VALUES (
      ...
      [
        ...
        tradeRecord.pct_of_volume, tradeRecord.execution_mode, tradeRecord.order_id,
        tradeRecord.status,
        tradeRecord.current_probability, tradeRecord.current_bid,
        tradeRecord.current_ask, tradeRecord.max_price_seen,
        tradeRecord.min_probability_seen, JSON.stringify(tradeRecord.evaluator_log),
    ...
    // Adjust bankroll (use actualCost for accurate accounting)
    if (isPws) {
      this.pwsGwBankroll -= actualCost;
    } else if (isMetarOnly) {
      if (isLive) {
        this.gwLiveBankroll -= actualCost;
      } else {
        this.gwPaperBankroll -= actualCost;
      }
    } else if (side === 'YES') {
      this.yesBankroll -= actualCost;
    } else {
      this.noBankroll -= actualCost;
      this.noExposureByDate[entry.target_date] = (this.noExposureByDate[entry.target_date] || 0) + actualCost;
    }

    const modeTag = executionMode === 'live' ? ' [LIVE]' : ' [PAPER]';
    this._log('info', `GUARANTEED TRADE${modeTag}: ${side} ${entry.city} ${entry.range_name} [${entry.platform}]`, {
      ask: actualAsk,
      shares,
    ...
    // Queue Telegram alert — include PWS-specific fields for display
    const fullTrade = {
      ...tradeRecord,
      id: tradeId,
      pws_corrected_median: entry.pws_corrected_median,
      pws_stations_online: entry.pws_stations_online,
      pws_avg_error: entry.pws_avg_error,
    };
    this.alerts.tradeEntry(fullTrade);
