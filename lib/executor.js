...
class Executor {
  ...
  async initBankrolls() {
    try {
      ...
      for (const t of openTrades || []) {
        ...
        if (t.entry_reason === 'guaranteed_win_metar_only') {
  ...
  async _executeGuaranteedSingle(entry) {
    ...
    // Determine live eligibility: GW_LIVE_ENABLED + polymarket + has tokenId
    const isLive = config.guaranteed_entry.GW_LIVE_ENABLED
      && entry.platform === 'polymarket'
      && entry.token_id;

    // Select matching bankroll
    const bankroll = isMetarOnly
      ? (isLive ? this.gwLiveBankroll : this.gwPaperBankroll)
      : (side === 'YES' ? this.yesBankroll : this.noBankroll);

    const bankrollLabel = isMetarOnly ? (isLive ? 'gwLive' : 'gwPaper') : side;
    ...
    const submitKey = `${entry.city}:${entry.target_date}:${entry.range_name}:${side}:${entry.platform}`;
    if (this._gwSubmitted.has(submitKey)) {
      this._log('info', `GW race dedup: already submitted ${entry.city} ${entry.range_name} ${side}`);
      return null;
    }
    this._gwSubmitted.add(submitKey);

    // Duplicate check against DB (check both live and paper)
    const { data: existing, error: dupError } = await query(
      `SELECT id FROM trades
       WHERE city = $1 AND target_date = $2 AND range_name = $3
         AND side = $4 AND platform = $5 AND status IN ($6, $7)
