...
class Executor {
  constructor(platformAdapter, alerts) {
    ...
    this.gwMetarBankroll = config.guaranteed_entry.GW_METAR_BANKROLL || 0;
  ...
  async initBankrolls() {
    try {
      const { data: openTrades, error } = await query(
        'SELECT side, cost, target_date, entry_reason FROM trades WHERE status = $1',
        ['open']
      );
      ...
      this.gwMetarBankroll = (config.guaranteed_entry.GW_METAR_BANKROLL || 0) - gwMetarDeployed;

      // NaN guard: validate bankrolls before allowing any trades
      if (!Number.isFinite(this.yesBankroll) || !Number.isFinite(this.noBankroll) || !Number.isFinite(this.gwMetarBankroll)) {
        this._log('error', 'Bankroll computed as NaN/Infinity — blocking all trades', {
          ...
          gwMetarBankroll: this.gwMetarBankroll,
          yesDeployed,
          noDeployed,
          gwMetarDeployed,
          YES_BANKROLL: config.sizing.YES_BANKROLL,
          NO_BANKROLL: config.sizing.NO_BANKROLL,
          GW_METAR_BANKROLL: config.guaranteed_entry.GW_METAR_BANKROLL,
        });
        this.yesBankroll = 0;
        this.noBankroll = 0;
        this.gwMetarBankroll = 0;
        this.bankrollValid = false;
        return;
      }
      ...
      this._log('info', 'Bankrolls initialized', {
        ...
        gwMetarBankroll: this.gwMetarBankroll.toFixed(2),
  ...
  async _executeGuaranteedSingle(entry) {
    ...
    const bankroll = isMetarOnly ? this.gwMetarBankroll : (side === 'YES' ? this.yesBankroll : this.noBankroll);

    if (!Number.isFinite(bankroll)) {
      this._log('error', `${isMetarOnly ? 'gwMetar' : side} bankroll is NaN — aborting guaranteed trade`);
      return null;
    }

    // Check bankroll
    if (bankroll <= config.sizing.MIN_BET) {
      this._log('warn', `Skipping guaranteed ${entry.city} ${entry.range_name} ${side}: ${isMetarOnly ? 'gwMetar' : side} bankroll depleted ($${bankroll.toFixed(2)})`);
      return null;
    }
