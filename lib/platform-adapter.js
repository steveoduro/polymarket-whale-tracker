/**
 * platform-adapter.js — Unified interface for Polymarket + Kalshi
 *
 * All platform-specific logic lives here. The rest of the bot is platform-agnostic.
 * Exposes: getMarkets, getPrice, getFeeRate, executeBuy, executeSell, getResolutionData
 */

const config = require('../config');
const crypto = require('crypto');
const https = require('https');

const GAMMA_API = 'https://gamma-api.polymarket.com';

// Polymarket slug generation
const MONTH_NAMES = ['january', 'february', 'march', 'april', 'may', 'june',
  'july', 'august', 'september', 'october', 'november', 'december'];

// Polymarket city slug mappings
const PM_CITY_SLUGS = {
  nyc: 'nyc', chicago: 'chicago', miami: 'miami', atlanta: 'atlanta',
  dallas: 'dallas', seattle: 'seattle', london: 'london', seoul: 'seoul',
  toronto: 'toronto', 'buenos aires': 'buenos-aires', ankara: 'ankara',
  wellington: 'wellington', paris: 'paris', 'sao paulo': 'sao-paulo',
};

// Kalshi series tickers per city (US-only; verified 2026-02-25)
const KALSHI_SERIES = {
  nyc: 'KXHIGHNY', chicago: 'KXHIGHCHI', miami: 'KXHIGHMIA',
  denver: 'KXHIGHDEN', austin: 'KXHIGHAUS', 'los angeles': 'KXHIGHLAX',
  atlanta: 'KXHIGHTATL', dallas: 'KXHIGHTDAL', seattle: 'KXHIGHTSEA',
  houston: 'KXHIGHTHOU', philadelphia: 'KXHIGHPHIL', dc: 'KXHIGHTDC',
  vegas: 'KXHIGHTLV', 'new orleans': 'KXHIGHTNOLA', 'san francisco': 'KXHIGHTSFO',
  phoenix: 'KXHIGHTPHX', boston: 'KXHIGHTBOS',
  'san antonio': 'KXHIGHTSATX', minneapolis: 'KXHIGHTMIN',
  'oklahoma city': 'KXHIGHTOKC',
};

const KALSHI_MONTH_MAP = { 0: 'JAN', 1: 'FEB', 2: 'MAR', 3: 'APR', 4: 'MAY', 5: 'JUN',
  6: 'JUL', 7: 'AUG', 8: 'SEP', 9: 'OCT', 10: 'NOV', 11: 'DEC' };

class PlatformAdapter {
  constructor() {
    this.kalshiKey = config.platforms.kalshi.apiKey;
    this.kalshiPrivateKey = null;
    this.lastKalshiRequest = 0;
    this.fetchModule = null; // lazy-loaded
    this.clobClient = null; // lazy-loaded Polymarket CLOB client
    // Cache Kalshi series data: key → { data, fetchedAt }
    this.kalshiCache = new Map();
    this.kalshiCacheTTL = 4 * 60 * 1000; // 4 minutes (shorter than 5m scan interval)
    // Cache Kalshi orderbook data: ticker → { bid_depth, ask_depth, fetchedAt }
    this.kalshiOrderbookCache = new Map();
    // Cache getMarkets() output: "city:dateStr" → { data, fetchedAt }
    this.marketsCache = new Map();
    this.marketsCacheTTL = 5 * 60 * 1000; // 5 minutes

    // Load Kalshi private key if configured
    if (config.platforms.kalshi.enabled && config.platforms.kalshi.privateKeyPath) {
      try {
        const fs = require('fs');
        this.kalshiPrivateKey = fs.readFileSync(config.platforms.kalshi.privateKeyPath, 'utf8');
      } catch (err) {
        console.log(`[WARN] Kalshi private key not loaded: ${err.message}`);
      }
    }
  }

  async _fetch(url, opts = {}) {
    if (!this.fetchModule) {
      this.fetchModule = (await import('node-fetch')).default;
    }
    // 15s timeout to prevent hung cycles
    if (!opts.signal) {
      opts.signal = AbortSignal.timeout(15000);
    }
    return this.fetchModule(url, opts);
  }

  _log(level, msg, data) {
    const ts = new Date().toISOString();
    const color = level === 'warn' ? '\x1b[33m' : '\x1b[36m';
    const label = level === 'warn' ? '[WARN]' : '[INFO]';
    const suffix = data ? ' ' + JSON.stringify(data) : '';
    console.log(`${ts} ${color}${label}\x1b[0m ${msg}${suffix}`);
  }

  /**
   * Get flat fee rate for a platform (legacy — returns 0 for both).
   * Use getEntryFee() for actual per-contract fee calculation.
   */
  getFeeRate(platform) {
    return config.platforms[platform]?.feeRate || 0;
  }

  /**
   * Calculate per-contract entry fee based on platform and price.
   * Kalshi: taker fee = 0.07 * price * (1-price), max ~1.75¢, charged on entry only (no settlement fee)
   * Polymarket: 0 (weather markets are fee-free)
   *
   * @param {string} platform
   * @param {number} price - contract price (0-1)
   * @returns {number} fee per contract in dollars
   */
  getEntryFee(platform, price) {
    if (platform === 'kalshi') {
      const multiplier = config.platforms.kalshi.takerFeeMultiplier || 0.07;
      return multiplier * price * (1 - price);
    }
    return 0;
  }

  // ══════════════════════════════════════════════════════════════════
  // UNIFIED INTERFACE
  // ══════════════════════════════════════════════════════════════════

  /**
   * Get all available markets for a city/date from all enabled platforms.
   * Returns normalized range objects.
   *
   * @param {string} city
   * @param {string} dateStr - YYYY-MM-DD
   * @returns {Array<{ platform, marketId, rangeName, rangeMin, rangeMax, rangeType, rangeUnit, bid, ask, spread, volume }>}
   */
  async getMarkets(city, dateStr) {
    const cacheKey = `${city}:${dateStr}`;
    const cached = this.marketsCache.get(cacheKey);
    if (cached && (Date.now() - cached.fetchedAt) < this.marketsCacheTTL) {
      return cached.data;
    }

    const results = [];

    const [pmRanges, klRanges] = await Promise.all([
      config.platforms.polymarket.enabled ? this._getPolymarketRanges(city, dateStr) : [],
      config.platforms.kalshi.enabled ? this._getKalshiRanges(city, dateStr) : [],
    ]);

    results.push(...pmRanges, ...klRanges);
    this.marketsCache.set(cacheKey, { data: results, fetchedAt: Date.now() });
    return results;
  }

  clearMarketsCache(city, dateStr) {
    if (city && dateStr) {
      this.marketsCache.delete(`${city}:${dateStr}`);
    } else {
      this.marketsCache.clear();
    }
  }

  /**
   * Get current price for a specific market/range.
   * @returns {{ bid, ask, spread, volume }}
   */
  async getPrice(platform, marketId) {
    if (platform === 'polymarket') {
      return this._getPolymarketPrice(marketId);
    } else if (platform === 'kalshi') {
      return this._getKalshiPrice(marketId);
    }
    return null;
  }

  /**
   * Lazy-init Polymarket CLOB client for live trading.
   * SignatureType 2 = GNOSIS_SAFE (proxy wallet: signer ≠ funder).
   */
  async _initClobClient() {
    const { ClobClient } = require('@polymarket/clob-client');
    const { Wallet } = require('ethers');

    const pk = process.env.WALLET_PRIVATE_KEY;
    if (!pk) throw new Error('WALLET_PRIVATE_KEY not set');

    const pmConfig = config.platforms.polymarket;
    if (!pmConfig.apiKey || !pmConfig.apiSecret || !pmConfig.apiPassphrase) {
      throw new Error('POLY_API_KEY/SECRET/PASSPHRASE not set — run scripts/derive-api-keys.js');
    }

    const signer = new Wallet(pk);
    const creds = {
      key: pmConfig.apiKey,
      secret: pmConfig.apiSecret,
      passphrase: pmConfig.apiPassphrase,
    };

    // SignatureType 2 = GNOSIS_SAFE (proxy wallet)
    const SIGNATURE_TYPE_GNOSIS = 2;
    const funder = pmConfig.funderAddress || signer.address;

    this.clobClient = new ClobClient(
      pmConfig.clobUrl,
      137, // Polygon chain ID
      signer,
      creds,
      SIGNATURE_TYPE_GNOSIS,
      funder
    );

    this._log('info', 'CLOB client initialized', { wallet: signer.address, funder });
  }

  /**
   * Buy — paper or live depending on opportunity.live flag.
   */
  async executeBuy(opportunity) {
    if (!opportunity.live) {
      return {
        paper: true,
        platform: opportunity.platform,
        price: opportunity.ask,
        shares: opportunity.shares,
        cost: opportunity.ask * opportunity.shares,
        timestamp: new Date().toISOString(),
      };
    }

    // Live execution on Polymarket CLOB
    if (!this.clobClient) await this._initClobClient();

    // Staleness guard: abort only if ask moved UP >5¢ (downward = better for us)
    // For GW fast-path: use pre-fetched CLOB book price (0-2s old, skips Gamma API)
    let currentAsk;
    if (opportunity._freshAskFromBook != null) {
      currentAsk = opportunity._freshAskFromBook;
    } else {
      const currentPrice = await this.getPrice(opportunity.platform, opportunity.marketId);
      currentAsk = currentPrice?.ask;
    }
    if (currentAsk != null && currentAsk > opportunity.ask + 0.05) {
      return {
        paper: false,
        aborted: true,
        reason: 'price_moved_up',
        detectedAsk: opportunity.ask,
        currentAsk,
      };
    }

    // Use lower of detected vs current ask (if price dropped, take it)
    const orderAsk = (currentAsk != null && currentAsk < opportunity.ask)
      ? currentAsk : opportunity.ask;

    const result = await this.clobClient.createAndPostOrder({
      tokenID: opportunity.tokenId,
      price: orderAsk,
      size: opportunity.shares,
      side: 'BUY',
    });

    return {
      paper: false,
      orderId: result.id,
      orderPrice: orderAsk,
      platform: opportunity.platform,
      shares: opportunity.shares,
      cost: orderAsk * opportunity.shares,
      timestamp: new Date().toISOString(),
    };
  }

  /**
   * Check order fill status on Polymarket CLOB.
   */
  async getOrderStatus(orderId) {
    if (!this.clobClient) await this._initClobClient();
    return await this.clobClient.getOrder(orderId);
  }

  /**
   * Cancel an open order on Polymarket CLOB.
   */
  async cancelOrder(orderId) {
    if (!this.clobClient) await this._initClobClient();
    return await this.clobClient.cancelOrder(orderId);
  }

  /**
   * Simulate selling at the bid (paper mode).
   */
  async executeSell(position) {
    const price = await this.getPrice(position.platform, position.marketId);
    if (!price) return null;
    return {
      paper: true,
      platform: position.platform,
      price: price.bid,
      shares: position.shares,
      revenue: price.bid * position.shares,
      timestamp: new Date().toISOString(),
    };
  }

  // ══════════════════════════════════════════════════════════════════
  // POLYMARKET
  // ══════════════════════════════════════════════════════════════════

  async _getPolymarketRanges(city, dateStr) {
    const slug = this._pmSlug(city, dateStr);
    if (!slug) return [];

    try {
      const event = await this._pmFetchEvent(slug);
      if (!event || !event.markets) return [];

      const cityConfig = config.cities[city.toLowerCase()];
      const unit = cityConfig?.unit || 'F';

      const ranges = event.markets.map(m => {
        const rangeName = m.groupItemTitle || m.question || '';
        const parsed = this._parseRange(rangeName, unit);
        if (parsed.rangeMin === undefined && parsed.rangeMax === undefined) return null;

        const bid = parseFloat(m.bestBid) || 0;
        let ask = parseFloat(m.bestAsk) || 0;

        // Fallback to outcomePrices
        if (ask === 0 && m.outcomePrices) {
          try {
            const prices = JSON.parse(m.outcomePrices);
            ask = parseFloat(prices[0]) || 0;
          } catch {}
        }

        return {
          platform: 'polymarket',
          marketId: m.id || m.conditionId,
          tokenId: m.clobTokenIds?.[0] || null,
          rangeName,
          rangeMin: parsed.rangeMin,
          rangeMax: parsed.rangeMax,
          rangeType: (parsed.rangeMin == null || parsed.rangeMax == null) ? 'unbounded' : 'bounded',
          rangeUnit: unit,
          bid,
          ask,
          spread: ask - bid,
          volume: parseFloat(m.volume) || 0,
          liquidity: parseFloat(m.liquidity) || 0,
          bid_depth: null,
          ask_depth: null,
        };
      }).filter(Boolean);

      // Fetch orderbook depth for Polymarket ranges that have a tokenId
      for (const range of ranges) {
        if (!range.tokenId) continue;
        const depth = await this._pmFetchOrderbook(range.tokenId);
        if (depth) {
          range.bid_depth = depth.bid_depth;
          range.ask_depth = depth.ask_depth;
        }
      }

      return ranges;
    } catch (err) {
      this._log('warn', `Polymarket fetch failed for ${city} ${dateStr}`, { error: err.message });
      return [];
    }
  }

  _pmSlug(city, dateStr) {
    const slugCity = PM_CITY_SLUGS[city.toLowerCase()];
    if (!slugCity) return null;

    const d = new Date(dateStr + 'T12:00:00Z');
    const month = MONTH_NAMES[d.getUTCMonth()];
    const day = d.getUTCDate();
    const year = d.getUTCFullYear();

    return `highest-temperature-in-${slugCity}-on-${month}-${day}-${year}`;
  }

  async _pmFetchEvent(slug) {
    try {
      const resp = await this._fetch(`${GAMMA_API}/events?slug=${encodeURIComponent(slug)}`);
      if (!resp.ok) return null;
      const events = await resp.json();
      return events[0] || null;
    } catch {
      return null;
    }
  }

  async _getPolymarketPrice(marketId) {
    try {
      const resp = await this._fetch(`${GAMMA_API}/markets/${marketId}`);
      if (!resp.ok) return null;
      const m = await resp.json();
      return {
        bid: parseFloat(m.bestBid) || 0,
        ask: parseFloat(m.bestAsk) || 0,
        spread: (parseFloat(m.bestAsk) || 0) - (parseFloat(m.bestBid) || 0),
        volume: parseFloat(m.volume) || 0,
      };
    } catch {
      return null;
    }
  }

  /**
   * Fetch orderbook depth from Polymarket CLOB API.
   * Returns top 3 levels of bid/ask depth.
   */
  async _pmFetchOrderbook(tokenId) {
    try {
      const resp = await this._fetch(
        `${config.platforms.polymarket.clobUrl}/book?token_id=${tokenId}`,
      );
      if (!resp.ok) return null;

      const data = await resp.json();
      // CLOB book returns: { bids: [{ price, size }, ...], asks: [{ price, size }, ...] }
      const bids = (data.bids || []).slice(0, 3).map(b => ({
        price: parseFloat(b.price),
        quantity: parseFloat(b.size),
      }));
      const asks = (data.asks || []).slice(0, 3).map(a => ({
        price: parseFloat(a.price),
        quantity: parseFloat(a.size),
      }));

      return { bid_depth: bids, ask_depth: asks };
    } catch {
      return null;
    }
  }

  // ══════════════════════════════════════════════════════════════════
  // KALSHI
  // ══════════════════════════════════════════════════════════════════

  async _getKalshiRanges(city, dateStr) {
    const series = KALSHI_SERIES[city.toLowerCase()];
    if (!series) return [];

    try {
      // Build event ticker: e.g., KXHIGHNY-26FEB12
      const d = new Date(dateStr + 'T12:00:00Z');
      const yearShort = String(d.getUTCFullYear()).slice(-2);
      const month = KALSHI_MONTH_MAP[d.getUTCMonth()];
      const day = String(d.getUTCDate()).padStart(2, '0');
      const eventTicker = `${series}-${yearShort}${month}${day}`;

      const markets = await this._klFetchMarkets(series);
      if (!markets || markets.length === 0) return [];

      // Filter to this event
      const eventMarkets = markets.filter(m => m.event_ticker === eventTicker);
      if (eventMarkets.length === 0) return [];

      // Fetch orderbook depth for each event market (parallel, rate-limited)
      const depthMap = new Map();
      for (const m of eventMarkets) {
        const depth = await this._klFetchOrderbook(m.ticker);
        if (depth) depthMap.set(m.ticker, depth);
      }

      return eventMarkets.map(m => {
        const yesBid = (m.yes_bid || 0) / 100;
        const yesAsk = (m.yes_ask || 0) / 100;
        const parsed = this._parseKalshiRange(m);
        if (!parsed) return null;

        const depth = depthMap.get(m.ticker);

        return {
          platform: 'kalshi',
          marketId: m.ticker,
          tokenId: m.ticker,
          rangeName: m.subtitle || m.yes_sub_title || `${parsed.rangeMin}-${parsed.rangeMax}°F`,
          rangeMin: parsed.rangeMin,
          rangeMax: parsed.rangeMax,
          rangeType: (parsed.rangeMin == null || parsed.rangeMax == null) ? 'unbounded' : 'bounded',
          rangeUnit: 'F',  // Kalshi is always °F
          bid: yesBid,
          ask: yesAsk,
          spread: yesAsk - yesBid,
          volume: m.volume || 0,
          liquidity: (m.liquidity || 0) / 100,
          bid_depth: depth?.bid_depth || null,
          ask_depth: depth?.ask_depth || null,
        };
      }).filter(Boolean);
    } catch (err) {
      this._log('warn', `Kalshi fetch failed for ${city} ${dateStr}`, { error: err.message });
      return [];
    }
  }

  _parseKalshiRange(market) {
    const floor = market.floor_strike;
    const cap = market.cap_strike;

    // Continuity correction: Kalshi resolves via NWS CLI (integer °F).
    // "above 35" = strictly > 35 = integer ≥ 36 = continuous ≥ 35.5
    // "below 26" = strictly < 26 = integer ≤ 25 = continuous ≤ 25.5
    // "between 33 and 35" = integer in [33,35] = continuous in [32.5, 35.5]
    if (market.strike_type === 'greater') {
      return { rangeMin: floor + 0.5, rangeMax: null };
    } else if (market.strike_type === 'less') {
      return { rangeMin: null, rangeMax: cap - 0.5 };
    } else if (market.strike_type === 'between') {
      return { rangeMin: floor - 0.5, rangeMax: cap + 0.5 };
    }
    return null;
  }

  async _klFetchMarkets(seriesTicker) {
    // Check cache first — same series data is needed for all 16 dates
    const cached = this.kalshiCache.get(seriesTicker);
    if (cached && (Date.now() - cached.fetchedAt) < this.kalshiCacheTTL) {
      return cached.data;
    }

    const apiUrl = config.platforms.kalshi.apiUrl;
    const allMarkets = [];
    let cursor = null;

    try {
      // Paginate through all results (limit 200 per page)
      do {
        // Rate limit: 8 req/sec
        const now = Date.now();
        const elapsed = now - this.lastKalshiRequest;
        if (elapsed < 125) {
          await new Promise(r => setTimeout(r, 125 - elapsed));
        }
        this.lastKalshiRequest = Date.now();

        let url = `${apiUrl}/markets?series_ticker=${seriesTicker}&status=open&limit=200`;
        if (cursor) url += `&cursor=${cursor}`;

        const headers = this.kalshiKey && this.kalshiPrivateKey
          ? this._klAuthHeaders('GET', '/trade-api/v2/markets')
          : { 'Accept': 'application/json' };

        const resp = await this._fetch(url, { headers });
        if (!resp.ok) throw new Error(`HTTP ${resp.status}`);

        const data = await resp.json();
        const markets = data.markets || [];
        allMarkets.push(...markets);

        cursor = data.cursor || null;
      } while (cursor);

      // Cache for subsequent date lookups within this cycle
      this.kalshiCache.set(seriesTicker, { data: allMarkets, fetchedAt: Date.now() });

      return allMarkets;
    } catch (err) {
      this._log('warn', `Kalshi markets fetch failed`, { series: seriesTicker, error: err.message });
      return allMarkets.length > 0 ? allMarkets : []; // Return partial results if available
    }
  }

  /**
   * Fetch orderbook depth for a Kalshi market ticker.
   * Returns top 3 levels of bid/ask depth, cached for 4 minutes.
   */
  async _klFetchOrderbook(ticker) {
    const cached = this.kalshiOrderbookCache.get(ticker);
    if (cached && (Date.now() - cached.fetchedAt) < this.kalshiCacheTTL) {
      return cached;
    }

    const apiUrl = config.platforms.kalshi.apiUrl;
    try {
      // Rate limit
      const now = Date.now();
      if (now - this.lastKalshiRequest < 125) {
        await new Promise(r => setTimeout(r, 125 - (now - this.lastKalshiRequest)));
      }
      this.lastKalshiRequest = Date.now();

      const headers = this.kalshiKey && this.kalshiPrivateKey
        ? this._klAuthHeaders('GET', `/trade-api/v2/markets/${ticker}/orderbook`)
        : { 'Accept': 'application/json' };

      const resp = await this._fetch(`${apiUrl}/markets/${ticker}/orderbook?depth=3`, { headers });
      if (!resp.ok) return null;

      const data = await resp.json();
      const ob = data.orderbook || {};

      // Parse YES bids (sorted ascending — last is best/highest)
      // In Kalshi binary markets: YES bids = buy orders for YES shares
      // YES ask depth = derived from NO bids (YES ask at price P = NO bid at 1-P)
      const yesBids = (ob.yes || []).map(([price, qty]) => ({
        price: price / 100,
        quantity: qty,
      }));
      const noBids = (ob.no || []).map(([price, qty]) => ({
        price: price / 100,
        quantity: qty,
      }));

      // Bid depth = YES bids (sorted descending by price = best first)
      const bid_depth = yesBids.reverse().slice(0, 3);
      // Ask depth = derived from NO bids (YES ask = 1 - NO bid price)
      const ask_depth = noBids.reverse().slice(0, 3).map(level => ({
        price: Math.round((1 - level.price) * 100) / 100,
        quantity: level.quantity,
      }));

      const result = { bid_depth, ask_depth, fetchedAt: Date.now() };
      this.kalshiOrderbookCache.set(ticker, result);
      return result;
    } catch (err) {
      // Non-critical — depth is nice-to-have
      return null;
    }
  }

  _klAuthHeaders(method, path) {
    const timestamp = Math.floor(Date.now() / 1000).toString();
    const message = timestamp + method.toUpperCase() + path;

    try {
      const sign = crypto.createSign('RSA-SHA256');
      sign.update(message);
      const signature = sign.sign(this.kalshiPrivateKey, 'base64');

      return {
        'Accept': 'application/json',
        'Content-Type': 'application/json',
        'KALSHI-ACCESS-KEY': this.kalshiKey,
        'KALSHI-ACCESS-SIGNATURE': signature,
        'KALSHI-ACCESS-TIMESTAMP': timestamp,
      };
    } catch {
      return { 'Accept': 'application/json' };
    }
  }

  async _getKalshiPrice(ticker) {
    const apiUrl = config.platforms.kalshi.apiUrl;
    try {
      const now = Date.now();
      if (now - this.lastKalshiRequest < 125) {
        await new Promise(r => setTimeout(r, 125 - (now - this.lastKalshiRequest)));
      }
      this.lastKalshiRequest = Date.now();

      const resp = await this._fetch(`${apiUrl}/markets/${ticker}`, {
        headers: { 'Accept': 'application/json' },
      });
      if (!resp.ok) return null;

      const data = await resp.json();
      const m = data.market || data;
      return {
        bid: (m.yes_bid || 0) / 100,
        ask: (m.yes_ask || 0) / 100,
        spread: ((m.yes_ask || 0) - (m.yes_bid || 0)) / 100,
        volume: m.volume || 0,
      };
    } catch {
      return null;
    }
  }

  // ══════════════════════════════════════════════════════════════════
  // RANGE PARSING (Polymarket format)
  // ══════════════════════════════════════════════════════════════════

  _parseRange(rangeStr, unit) {
    // Continuity correction: both platforms resolve to whole-degree integers.
    // A range like "34-35°F" means the recorded integer is 34 or 35,
    // which maps to continuous temp in [33.5, 35.5). Apply ±0.5 to all bounds.

    // "≤17°F" / "below" / "or less" / "or below"
    if (/≤|below|or\s+(less|below)/i.test(rangeStr)) {
      const num = rangeStr.match(/-?[\d.]+/);
      if (num) return { rangeMin: null, rangeMax: parseFloat(num[0]) + 0.5 };
    }

    // "≥28°F" / "above" / "or higher" / "or more"
    if (/≥|higher|above|or\s+more/i.test(rangeStr)) {
      const num = rangeStr.match(/-?[\d.]+/);
      if (num) return { rangeMin: parseFloat(num[0]) - 0.5, rangeMax: null };
    }

    // "18-19°F" or "18–19°F" (hyphen or en-dash)
    const rangeMatch = rangeStr.match(/(-?[\d.]+)\s*[-–]\s*(-?[\d.]+)/);
    if (rangeMatch) {
      return { rangeMin: parseFloat(rangeMatch[1]) - 0.5, rangeMax: parseFloat(rangeMatch[2]) + 0.5 };
    }

    // Single value "6°C" — already has ±0.5 (integer resolves from [n-0.5, n+0.5))
    const single = rangeStr.match(/(-?[\d.]+)\s*°/);
    if (single) {
      const n = parseFloat(single[1]);
      return { rangeMin: n - 0.5, rangeMax: n + 0.5 };
    }

    return {};
  }
}

module.exports = PlatformAdapter;
module.exports._KALSHI_SERIES = KALSHI_SERIES;
module.exports._PM_CITY_SLUGS = PM_CITY_SLUGS;
