...
class ForecastEngine {
  ...
  async getForecast(city, dateStr) {
    ...
    if (nwsBoost && cityConfig.kalshiNwsPriority && activeSourceKeys.includes('nws')) {
      const nwsIdx = activeSourceKeys.indexOf('nws');
      if (cityWeights && activeSourceKeys.every(k => cityWeights[k])) {
        // Boost NWS weight and renormalize
        const boosted = {};
        for (const k of activeSourceKeys) {
          boosted[k] = cityWeights[k] * (k === 'nws' ? nwsBoost : 1);
        }
        const bTotal = Object.values(boosted).reduce((a, b) => a + b, 0);
        let wSum = 0;
        for (let i = 0; i < activeSourceKeys.length; i++) {
          wSum += correctedTempsF[i] * (boosted[activeSourceKeys[i]] / bTotal);
        }
        kalshiAvgF = wSum;
      } else {
        // No weights yet — give NWS 60% and distribute remainder equally among others
        const nwsW = 0.6;
        const otherW = (1 - nwsW) / (activeSourceKeys.length - 1);
        let wSum = 0;
        for (let i = 0; i < activeSourceKeys.length; i++) {
