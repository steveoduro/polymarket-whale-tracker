...
class ForecastEngine {
  ...
  async _loadCalibrationFromDB() {
    ...
    // ── Layer 1: Per-city MAE (for city eligibility gate) ──
    const cityAbsErrors = {};
    for (const row of data) {
      const ck = row.city?.toLowerCase();
      if (!ck || row.abs_error == null) continue;
      if (!cityAbsErrors[ck]) cityAbsErrors[ck] = { errors: [], unit: row.unit };
      cityAbsErrors[ck].errors.push(row.abs_error);
    }

    const cityMAE = {};
    for (const [ck, { errors, unit }] of Object.entries(cityAbsErrors)) {
      const n = errors.length;
      const mae = errors.reduce((a, b) => a + b, 0) / n;
      cityMAE[ck] = { mae: Math.round(mae * 100) / 100, n, unit };
    }

    // ── Layer 2: Per-city per-source MAE ranking + active source sets ──
    ...
    const WEIGHT_MIN = SM.WEIGHT_MIN_SAMPLES || 5;
    ...
    // ── Layer 4: Per-city inverse-MAE source weights ──
    ...
    for (const [ck, rankings] of Object.entries(citySourceRanking)) {
      ...
      qualified.forEach((r, i) => {
        citySourceWeights[ck][r.source] = Math.round((inverseMAEs[i] / total) * 1000) / 1000;
      });
    }
    ...
    for (const [ck, rankings] of Object.entries(citySourceRanking)) {
      ...
      const parts = rankings.map(r => {
        const status = active.has(r.source) ? '' : ' DEMOTED';
