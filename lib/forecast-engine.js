...
class ForecastEngine {
  ...
  async getForecast(city, dateStr) {
    ...
    // Base std dev values represent day-1 accuracy; uncertainty grows with sqrt(days)
    const hoursToResolution = this._getHoursToResolution(dateStr, cityConfig.tz);
    if (hoursToResolution !== null) {
      const daysOut = Math.max(0.5, hoursToResolution / 24); // floor at half-day for same-day
  ...
  _getHoursToResolution(dateStr, timezone) {
    try {
      const now = Date.now();
      // Build resolution time: target_date at 23:00 local time
      // Use Intl to get the UTC offset for the target timezone
      const targetLocal = new Date(dateStr + 'T23:00:00');
      ...
      // Get current time in target timezone to compute offset
      const nowUTC = new Date();
      const parts = formatter.formatToParts(nowUTC);
      const getPart = (type) => parseInt(parts.find(p => p.type === type)?.value || '0');
      const localNow = new Date(getPart('year'), getPart('month') - 1, getPart('day'),
        getPart('hour'), getPart('minute'), getPart('second'));
      const offsetMs = nowUTC.getTime() - localNow.getTime();

      // Resolution time in UTC = target_date 23:00 local + offset
      const resolutionUTC = new Date(dateStr + 'T23:00:00');
      resolutionUTC.setTime(resolutionUTC.getTime() + offsetMs);
