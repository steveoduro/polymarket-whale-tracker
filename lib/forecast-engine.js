...
class ForecastEngine {
  ...
  async getForecast(city, dateStr) {
    ...
    if (ensembleSpread) {
      ...
      db.from('ensemble_spread').upsert({
        city: cityKey, target_date: dateStr,
        ensemble_std_c: ensembleSpread.stdC,
        ensemble_mean_c: ensembleSpread.meanC,
        member_count: ensembleSpread.count,
        member_min_c: ensembleSpread.minC,
        member_max_c: ensembleSpread.maxC,
        deterministic_c: deterministicC != null ? Math.round(deterministicC * 100) / 100 : null,
        hours_to_resolution: hoursToResolution != null ? Math.round(hoursToResolution * 10) / 10 : null,
      }, { onConflict: 'city,target_date' }).then(() => {}).catch(() => {});
    }
  ...
  async getCityEligibility(cityKey) {
    ...
    // Prefer ensemble_corrected MAE when available (reflects full correction pipeline)
    let cityData = cal.cityMAE?.[cityKey.toLowerCase()];
    if (CE.PREFER_ENSEMBLE_MAE !== false) {
      const ensembleData = cal.cityEnsembleMAE?.[cityKey.toLowerCase()];
      if (ensembleData && ensembleData.n >= minSamples) {
        cityData = ensembleData;
      }
    }
    ...
    try {
      ...
      if (!this.calibration) {
        this.calibration = {
          ...
          cityMAE: {}, cityEnsembleMAE: {}, cityActiveSources: {}, citySourceWeights: {},
  ...
  async _loadCalibrationFromDB() {
    ...
    try {
      const { data: ensRows, error: ensErr } = await db
        .from('v2_forecast_accuracy')
        .select('city, unit, error, abs_error')
        .gte('target_date', cutoffStr)
    ...
    return { biases, empiricalStdDevs, cityStdDevs, leadBiases,
             cityMAE, cityEnsembleMAE, cityActiveSources, citySourceWeights, cityBiases, cityLeadBiases, citySourceRanking };
  }
  ...
  async _loadCityDistributions() {
    try {
      const { data, error } = await db.from('city_error_distribution').select('*');
