...
class ForecastEngine {
  ...
  async getForecast(city, dateStr) {
    ...
    // ── Bias correction (lead-time-aware) ──────────────────────────
    ...
    // Uses lead-time-specific bias when available, falls back to all-lead-time pooled.
    ...
    const leadBuckets = config.forecasts.LEAD_TIME_BUCKETS || [
      { name: 'short', min: 0, max: 48 },
      { name: 'medium', min: 49, max: 120 },
      { name: 'long', min: 121, max: Infinity },
    ];
    let leadBucket = null;
    ...
    for (const srcKey of activeSourceKeys) {
      ...
      // Try lead-time-specific bias first, fall back to all-lead-time pooled
      const leadBiasKey = leadBucket ? `${srcKey}:${unit}:${leadBucket}` : null;
      const leadBiasEntry = leadBiasKey ? cal.leadBiases?.[leadBiasKey] : null;
      const biasEntry = (leadBiasEntry && leadBiasEntry.n >= MIN_BIAS_SAMPLES)
        ? leadBiasEntry : cal.biases[biasKey];
      let correctedF = rawF;
  ...
  async _loadCalibrationFromDB() {
    ...
    const cityKeys = Object.entries(cityStdDevs)
    ...
    this.log('info', 'Calibration loaded', {
      ...
      cityStdDevs: cityKeys.length > 0 ? cityKeys.join(', ') : 'none (insufficient data)',
