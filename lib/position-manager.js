...
class PositionManager {
  constructor(config = {}) {
    ...
    // Early actuals (METAR) — log-only mode for initial deployment
    this.earlyActualsEnabled = this.config.EARLY_ACTUALS_ENABLED ?? true;
    this.earlyActualsLogOnly = this.config.EARLY_ACTUALS_LOG_ONLY ?? true;
    this.earlyActualMinBid = this.config.EARLY_ACTUAL_MIN_BID ?? 0.03;
    ...
    try {
      ...
      for (const position of openPositions) {
        try {
          ...
          // === EARLY ACTUALS CHECK (same-day positions only) ===
          if (this.earlyActualsEnabled) {
            const daysToRes = this.getDaysToResolution(position.target_date);
            if (daysToRes === 0) {
              const actual = await this.weatherApi.getCurrentDayHigh(position.city);
              if (actual) {
                const rangeBounds = this.parseRangeBounds(position.range_name);
                if (rangeBounds) {
                  const status = this.getEarlyActualStatus(actual, rangeBounds);

                  // CONFIRMED WIN ("or higher" only) — block take-profit, ride to $1.00
                  if (status === 'confirmed_win') {
                    const ptag = (position.platform || 'polymarket') === 'kalshi' ? '[KL] ' : '[PM] ';
                    await this.logAction(position, 'early_actual_confirmed_win', currentPrice, {
                      actual_high_f: actual.highF,
                      local_hour: actual.localHour,
                      threshold: rangeBounds.threshold,
                      reason: `Actual ${actual.highF}°F >= ${rangeBounds.threshold}°F threshold — riding to resolution`,
                    });
                    if (!this.earlyActualsLogOnly) {
                      this.log('success', 'Confirmed winner — riding to resolution', {
                        city: position.city, range: position.range_name,
                        actual: actual.highF + '°F', threshold: rangeBounds.threshold + '°F',
                      });
                      const entryPrice = parseFloat(position.entry_price);
                      const shares = parseFloat(position.shares);
                      const expectedPnl = (1 - entryPrice) * shares * (1 - this.getFeeRate(position));
                      const localTimeStr = actual.localHour >= 12 ? (actual.localHour === 12 ? 12 : actual.localHour - 12) + ' PM' : (actual.localHour === 0 ? 12 : actual.localHour) + ' AM';
                      await this.sendTelegram(
                        `🎯 *[Bot B] ${ptag}RIDING TO RESOLUTION*\n\n` +
                        `📍 ${position.city.toUpperCase()} - ${position.target_date}\n` +
                        `🌡️ Running high: ${actual.highF}°F (as of ${localTimeStr} local)\n` +
                        `📊 Status: confirmed_win (>= ${rangeBounds.threshold}°F threshold)\n\n` +
                        `Position: $${parseFloat(position.cost).toFixed(2)} @ ${(entryPrice * 100).toFixed(0)}¢\n` +
                        `Current bid: ${(currentPrice.bid * 100).toFixed(0)}¢ (would have TP'd)\n` +
                        `Expected: $1.00 at resolution → +$${expectedPnl.toFixed(2)}`
                      );
                    }
                    // Always skip take-profit for confirmed winners (even in log-only mode)
                    monitored++;
                    continue;
                  }

                  // CONFIRMED/PROBABLE LOSS — exit to salvage value
                  if (status === 'confirmed_loss' || status === 'probable_loss') {
                    if (currentPrice.bid >= this.earlyActualMinBid) {
                      if (this.earlyActualsLogOnly) {
                        await this.logAction(position, 'early_actual_would_exit', currentPrice, {
                          actual_high_f: actual.highF,
                          local_hour: actual.localHour,
                          status,
                          reason: `LOG ONLY: Would exit (${status}), bid=${(currentPrice.bid * 100).toFixed(0)}¢`,
                        });
                      } else {
                        await this.executeEarlyActualExit(position, currentPrice, actual, status);
                        exits++;
                        continue;
                      }
                    }
                  ...
                  // All other statuses: log and continue normal Bot B logic
                  if (status !== 'too_early') {
                    await this.logAction(position, 'early_actual_monitor', currentPrice, {
                      actual_high_f: actual.highF,
                      local_hour: actual.localHour,
                      high_likely_final: actual.highLikelyFinal,
                      status,
                    });
                  }
                }
  ...
  /**
   * Parse range name into structured bounds.
   * Handles: "54-55°F", "51° to 52°", "72°F or higher", "32°F or below", "6°C", "24-25°C"
   * @param {string} rangeName
   * @returns {Object|null} { type, low, high, threshold, value, unit }
   */
  parseRangeBounds(rangeName) {
    const cleaned = rangeName.replace(/Â/g, '');
    const unit = cleaned.includes('°C') ? 'C' : 'F';

    // "X or higher/above"
    if (/higher|above/i.test(cleaned)) {
      const threshold = parseFloat(cleaned.match(/-?[\d.]+/)?.[0]);
      if (isNaN(threshold)) return null;
      return { type: 'or_higher', threshold, unit };
    }
    ...
    if (/below/i.test(cleaned)) {
      const threshold = parseFloat(cleaned.match(/-?[\d.]+/)?.[0]);
      if (isNaN(threshold)) return null;
      return { type: 'or_below', threshold, unit };
    }

    // "X-Y" or "X–Y" (en-dash) or "X to Y" bounded range
    const rangeMatch = cleaned.match(/(-?[\d.]+)\s*(?:[-–]|to)\s*(-?[\d.]+)/);
    if (rangeMatch) {
      return { type: 'bounded', low: parseFloat(rangeMatch[1]), high: parseFloat(rangeMatch[2]), unit };
    }

    // Single value "X°F" or "X°C"
    const single = cleaned.match(/(-?[\d.]+)\s*°/);
    if (single) {
      return { type: 'single', value: parseFloat(single[1]), unit };
    }
  ...
  /**
   * Determine early actual status given a running daily high and parsed range bounds.
   * Pure function — no side effects.
   * @param {Object} actual - { highF, highC, highLikelyFinal }
   * @param {Object} rangeBounds - from parseRangeBounds()
   * @returns {string} Status: 'confirmed_win', 'confirmed_loss', 'probable_win', 'probable_loss', 'in_range_early', 'too_early'
   */
  getEarlyActualStatus(actual, rangeBounds) {
    // Convert actual to native unit for comparison
    let temp = actual.highF;
    if (rangeBounds.unit === 'C') {
      temp = (actual.highF - 32) * 5 / 9;
    }

    const { type, low, high, threshold } = rangeBounds;
    const final = actual.highLikelyFinal;

    if (type === 'bounded') {
      if (temp > high) return 'confirmed_loss';     // Exceeded upper bound, can't un-exceed
      if (temp >= low && temp <= high) {
        return final ? 'probable_win' : 'in_range_early';
      }
      // temp < low
      if (final && (low - temp) >= 2) return 'probable_loss';
      return 'too_early';
    }

    if (type === 'or_higher') {
      if (temp >= threshold) return 'confirmed_win';  // Hit threshold, daily high only goes up
      if (final && (threshold - temp) >= 2) return 'probable_loss';
      return 'too_early';
    }

    if (type === 'or_below') {
      if (temp > threshold) return 'confirmed_loss';  // Exceeded threshold
      return final ? 'probable_win' : 'too_early';
    }
  ...
  /**
   * Execute an early actual exit (confirmed/probable loss — salvage remaining value)
   */
  async executeEarlyActualExit(position, currentPrice, actual, status) {
    const exitPrice = currentPrice.bid;
    const shares = parseFloat(position.shares);
    const entryPrice = parseFloat(position.entry_price);
    const grossProfit = (exitPrice - entryPrice) * shares;
    const fee = exitPrice * shares * this.getFeeRate(position);
    const netPnl = grossProfit - fee;
    const wouldLose = -parseFloat(position.cost);
    ...
    if (error) {
      this.log('error', 'Failed to update trade for early actual exit', { error: error.message });
      return;
    }

    const label = isReentry ? 'EARLY ACTUAL EXIT (re-entry)' : 'EARLY ACTUAL EXIT';
    this.log('warn', `${label} executed`, {
      city: position.city,
      date: position.target_date,
      range: position.range_name,
      actualHighF: actual.highF,
      localHour: actual.localHour,
      status,
      exitPrice: (exitPrice * 100).toFixed(0) + '¢',
      netPnl: '$' + netPnl.toFixed(2),
      savedVsResolution: '$' + (netPnl - wouldLose).toFixed(2),
    });

    await this.logAction(position, 'exit_early_actual', currentPrice, {
      actual_high_f: actual.highF,
      local_hour: actual.localHour,
      status,
      netPnl,
      saved_vs_resolution: netPnl - wouldLose,
      reason: `${status}: actual ${actual.highF}°F vs ${position.range_name} — salvaged ${(exitPrice * 100).toFixed(0)}¢`,
      source: table,
    });
    ...
    if (this.telegramOnExit) {
      ...
      const salvaged = exitPrice * shares * (1 - this.getFeeRate(position));
      await this.sendTelegram(
        `🌡️ *[Bot B] ${ptag}${label}*\n\n` +
        `📍 ${position.city.toUpperCase()} - ${position.target_date}\n` +
        `🌡️ Running high: ${actual.highF}°F (as of ${actual.localHour >= 12 ? (actual.localHour === 12 ? 12 : actual.localHour - 12) + ' PM' : (actual.localHour === 0 ? 12 : actual.localHour) + ' AM'} local)\n` +
        `📊 Status: ${status} (${position.range_name})\n\n` +
        `Position: $${cost.toFixed(2)} @ ${(entryPrice * 100).toFixed(0)}¢\n` +
        `Exit: ${(exitPrice * 100).toFixed(0)}¢ bid → salvaged $${salvaged.toFixed(2)}\n` +
        `P&L: $${netPnl.toFixed(2)} (vs -$${cost.toFixed(2)} at resolution)`
      );
    }
  }
  ...
  async executeReentry(exitedPosition, currentPrice, forecast, edge, previousEdge = 0) {
    ...
    const effectivePayout = 1 - this.getFeeRate(exitedPosition);
  ...
  async getBankroll() {
    ...
    // Include re-entry P&L in bankroll calculation
    const { data: reentryData } = await this.supabase
      .from('reentry_trades')
      .select('pnl')
      .in('status', ['won', 'lost', 'exited']);

    const reentryPnl = (reentryData || []).reduce((sum, t) => sum + (parseFloat(t.pnl) || 0), 0);

    return this.paperBankroll + realizedPnl + reentryPnl;
  }
  ...
  async resolveReentryTrades() {
    ...
    for (const reentry of reentries) {
      try {
        ...
        await this.logAction({ ...reentry, _source: 'reentry_trades' }, 'resolution', { bid: won ? 1.0 : 0.0, ask: won ? 1.0 : 0.0 }, {
          reentry_id: reentry.id,
