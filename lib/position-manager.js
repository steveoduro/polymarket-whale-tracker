...
class PositionManager {
  ...
  async executeTakeProfit(position, currentPrice, entryTier, exitThreshold) {
    ...
    if (this.telegramOnExit) {
      ...
      await this.sendTelegram(
        `🎯 *[Bot B] TAKE PROFIT*: ${position.city} ${position.range_name}\n` +
  ...
  async executeForecastExit(position, currentPrice, forecast) {
    ...
    if (this.telegramOnExit) {
      await this.sendTelegram(
        `⚠️ *[Bot B] FORECAST EXIT*: ${position.city} ${position.range_name}\n` +
  ...
  async checkReentryOpportunities() {
    // Get recently forecast-exited positions (last 24 hours) that haven't resolved yet
    // Only consider forecast_shift exits - take_profit exits already captured value
    ...
    for (const position of exitedPositions) {
      try {
        // Dedup by city + date + range (not original_trade_id)
        ...
        // Edge must be higher than original entry edge
        const { data: origOpp } = await this.supabase
          .from('weather_opportunities')
          .select('mispricing_pct')
          .eq('city', position.city)
          .eq('target_date', position.target_date)
          .eq('status', 'traded')
        ...
        const originalEdgePct = origOpp && origOpp.length > 0 ? parseFloat(origOpp[0].mispricing_pct) : 0;
        const reentryEdgePct = edge * 100;

        if (reentryEdgePct <= originalEdgePct) {
          await this.logAction(position, 'skip_reentry', currentPrice, {
            reason: `Re-entry edge ${reentryEdgePct.toFixed(1)}% not higher than original ${originalEdgePct.toFixed(1)}% - skipping`,
          });
          this.log('info', 'Re-entry edge not higher than original', {
            city: position.city,
            range: position.range_name,
            reentryEdge: reentryEdgePct.toFixed(1) + '%',
            originalEdge: originalEdgePct.toFixed(1) + '%',
          });
          continue;
        }

        // Execute re-entry
        await this.executeReentry(position, currentPrice, forecast, edge, originalEdgePct);
   ...
   */
  async executeReentry(originalPosition, currentPrice, forecast, edge, originalEdgePct = 0) {
    ...
    // Cap at 2x original position cost
    const originalCost = parseFloat(originalPosition.cost) || positionSize;
    ...
    this.log('success', 'RE-ENTRY executed', {
      ...
      edge: reentryEdgePct + '%',
      originalEdge: originalEdgePct.toFixed(1) + '%',
      edgeImprovement: (edge * 100 - originalEdgePct).toFixed(1) + '%',
    ...
    if (this.telegramOnReentry) {
      await this.sendTelegram(
        `🔄 *[Bot B] RE-ENTRY*: ${originalPosition.city} ${originalPosition.range_name}\n` +
        `Original exit: ${(parseFloat(originalPosition.exit_price) * 100).toFixed(0)}¢ | Original cost: $${originalCost.toFixed(2)}\n` +
        `Re-entry: ${(entryPrice * 100).toFixed(0)}¢ | Cost: $${positionSize.toFixed(2)} (cap: $${maxReentryCost.toFixed(2)})\n` +
        `Edge: ${reentryEdgePct}% (was ${originalEdgePct.toFixed(1)}%, +${(edge * 100 - originalEdgePct).toFixed(1)}%)`
      );
    }
  }
