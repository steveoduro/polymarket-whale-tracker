...
class PositionManager {
  ...
  async logAction(position, action, currentPrice, extra = {}) {
    try {
      ...
      // If details column doesn't exist yet (pre-migration), retry without it
      if (error && details && error.message && error.message.includes('details')) {
        delete insertFields.details;
        await this.supabase
          .from('position_manager_logs')
          .insert(insertFields);
      }
